...
import { getResolvedTheme, ThemePreference } from "@/components/site/ThemeToggle";
...
export const Route = createFileRoute("/aeroforge")({
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "AeroForge AI Simulation Workstation — Project Polaris" },
      {
...
function AeroForgeLab() {
  ...
  useEffect(() => {
    ...
    // Safety timeout to ensure loading state dismisses gracefully
  ...
  const handleReloadIframe = () => {
    ...
    if (iframeRef.current) {
      iframeRef.current.src = `/aeroforge/index.html?t=${Date.now()}`;
    }
  };
    ...
    <div
      className={`relative w-full bg-background flex flex-col ${
        isFullscreen
          ...
          : "h-[calc(100dvh-4rem)] md:h-[calc(100dvh-4.5rem)] pt-16 md:pt-20"
      }`}
    >
      {/* Transparency & Security Modal */}
      <TransparencyModal isOpen={showTransparency} onClose={() => setShowTransparency(false)} />
      ...
      <div className="w-full bg-surface border-b border-border px-4 md:px-6 py-2 flex items-center justify-between text-xs font-mono text-muted-foreground shrink-0 z-20">
        ...
        <div className="flex items-center gap-2 shrink-0 ml-3">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-surface-2 hover:bg-surface-3 text-foreground/80 hover:text-foreground border border-border text-[11px] transition-colors"
          ...
          <button
            ...
            aria-label={
              isFullscreen ? "Exit Fullscreen mode" : "Expand AeroForge Lab to Full Viewport"
            }
      ...
      <div className="relative flex-1 w-full h-full overflow-hidden bg-background">
        {/* Loading Overlay */}
        {isLoading && (
          <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-background/95 backdrop-blur-sm text-foreground font-mono text-xs space-y-3">
            ...
            <p className="text-[11px] text-muted-foreground">
              Secured via Project Polaris Sandbox Environment
            </p>
          </div>
        )}

        {/* Graceful Fallback if frame error occurs */}
        {hasLoadError && (
          <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-card p-6 text-center font-sans space-y-4">
            ...
            <div className="space-y-1 max-w-md">
              <h3 className="font-bold text-foreground text-base">AeroForge Standalone Access</h3>
              <p className="text-xs text-muted-foreground">
                If the embedded physics canvas does not load inside this container, you can launch
                AeroForge directly in a standalone browser tab.
              </p>
            </div>
            <div className="flex items-center gap-3">
              <Button size="sm" onClick={handleReloadIframe} className="gap-1.5 text-xs">
                <RefreshCw className="size-3.5" />
                <span>Retry Canvas</span>
              </Button>
              <Button asChild size="sm" variant="outline" className="gap-1.5 text-xs">
                <a href="/aeroforge/" target="_blank" rel="noopener noreferrer">
                  <ExternalLink className="size-3.5 text-primary" />
                  <span>Open Standalone Lab</span>
                </a>
              </Button>
            </div>
          </div>
