...
export interface ProjectWorkspace {
  projectId: string;
  activeTab: 'overview' | 'requirements' | 'comparison' | 'provenance' | 'notebook' | 'simulations' | 'datasets' | 'results' | 'validation';
...
interface ProjectStore {
  ...
  updateRequirement: (projectId: string, reqId: string, updates: Partial<ProjectRequirement>) => void;
...
export const INITIAL_DEMO_PROJECTS: Project[] = [
  {
    ...
    requirements: [
      { id: 'req_1', code: 'REQ-AERO-01', category: 'Aerodynamics', specification: 'Cruise Lift-to-Drag Ratio (L/D)', targetValue: '> 14.5', currentValue: '15.2 (Verified)', status: 'VERIFIED' },
      { id: 'req_2', code: 'REQ-AERO-02', category: 'Aerodynamics', specification: 'Maximum Lift Coefficient CL_max', targetValue: '> 1.20', currentValue: '1.34 (Verified)', status: 'VERIFIED' }
    ],
    simulations: [
      { id: 'sim_1', name: 'NACA 0012 Subsonic AoA Sweep', domain: 'Aerodynamics', solver: 'Thin Airfoil + PG Correction', status: 'COMPLETED', date: '2026-08-10', metrics: { CL: 0.44, CD: 0.008, 'L/D': 55.0 } }
    ],
    notebooks: [
      { id: 'nb_1', title: 'Thin Airfoil Polar Analysis Notes', author: 'Aerospace Researcher', lastUpdated: '2026-08-11', content: 'Evaluated NACA 0012 section properties under subcritical flow.' }
    ]
  },
  {
    ...
    requirements: [
      { id: 'req_uav_1', code: 'REQ-UAV-01', category: 'Optimization', specification: 'Morphing L/D Target', targetValue: '> 18.0', currentValue: '17.8 (In Progress)', status: 'IN_PROGRESS' }
    ]
  },
  {
    ...
    requirements: [
      { id: 'req_bm_1', code: 'REQ-STRUCT-01', category: 'Structures', specification: 'Tip Deflection Limit', targetValue: '< 15.0 mm', currentValue: '11.8 mm (Verified)', status: 'VERIFIED' }
    ]
  }
];
  ...
  persist(
    (set, get) => ({
      ...
      deleteProject: (id) =>
        set((state) => {
          ...
          const nextCurrent = state.currentProject?._id === id ? (filtered[0] || null) : state.currentProject;
          return {
            projects: filtered,
      ...
      updateRequirement: (projectId, reqId, updates) =>
        set((state) => {
          const updatedProjects = state.projects.map((p) => {
            ...
            return {
              ...p,
              requirements: (p.requirements || []).map((r) => (r.id === reqId ? { ...r, ...updates } : r)),
              updatedDate: new Date().toISOString(),
            };
          });
          const updatedCurrent =
    ...
    }),
    {
      ...
      migrate: (persistedState: any, version: number) => {
        if (!persistedState || !Array.isArray(persistedState.projects) || persistedState.projects.length === 0) {
          return {
            currentProject: INITIAL_DEMO_PROJECTS[0],
