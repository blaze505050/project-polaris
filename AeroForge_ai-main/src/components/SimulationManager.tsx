...
export default function SimulationManager({ projectId }: SimulationManagerProps) {
  ...
  const [simulations, setSimulations] = useState<SimulationJob[]>([
    ...
    },
    {
      ...
      estCompletion: 0,
      meshSize: '3.1M cells',
      residuals: [1e-1, 2e-1, 8e-1, 4.5e0],
      logs: [
  ...
  const handleCreateSimulation = () => {
    ...
    const newSim: SimulationJob = {
      ...
      meshSize: '3.5M cells',
      residuals: [],
      logs: [`[${new Date().toLocaleTimeString()}] Job queued successfully in AeroForge compute cluster.`],
      createdAt: new Date(),
    };
        ...
        <div className="bg-[#080E1C] border border-white/10 rounded-lg p-3.5 flex items-center justify-between">
          <div>
            ...
            <div className="text-xl font-bold text-cyan-400 mt-0.5">
              {simulations.filter((s) => s.status === 'RUNNING' || s.status === 'CONVERGING').length} / 8
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div className="bg-[#080E1C] border border-white/10 rounded-lg p-4 space-y-4">
          {selectedSim ? (
                  ...
                  {selectedSim.residuals.length > 0 ? (
                    selectedSim.residuals.map((r, i) => (
                      <div
                        ...
                        style={{ height: `${Math.min(100, Math.max(10, Math.log10(1 / r) * 20))}%` }}
            ...
            <div className="py-12 text-center text-white/40">Select a simulation to view workspace.</div>
          )}
        </div>
      </div>
