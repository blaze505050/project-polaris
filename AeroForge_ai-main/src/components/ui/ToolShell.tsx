...
function formatForExport(name: string, inputs: Record<string, number>, inputDefs: ToolInput[], results: ToolResult[]) {
  ...
  return {
    ...
    json: JSON.stringify({
      tool: name,
      timestamp,
      disclaimer: 'Simplified model for conceptual analysis only. Not certified for production use.',
      ...
      results: Object.fromEntries(results.map((r) => [r.label, { value: r.value, unit: r.unit }])),
    }, null, 2),
  };
}
...
export default function ToolShell({
  ...
  const handleInputChange = useCallback((key: string, raw: string) => {
    ...
    setTimeout(() => {
      try {
        ...
        if (!output) {
          setCalcState('error');
          setResults(null);
          setInterpretation('Calculation returned no results. Check inputs are within valid ranges.');
          return;
        }
        ...
        <div>
          <h2 className="text-lg font-bold text-white flex items-center gap-2">
            ...
            <span
              ...
              style={{ backgroundColor: statusColors[status] + '20', color: statusColors[status], border: `1px solid ${statusColors[status]}40` }}
            >
              {status}
            </span>
          </h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="space-y-3">
          {Array.from(groups.entries()).map(([groupName, groupInputs]) => (
            <div key={groupName} className="bg-[#0A1020] border border-white/8 rounded-lg p-4 space-y-3">
              <h3 className="text-xs font-bold text-white/60 uppercase tracking-wider">{groupName}</h3>
              {groupInputs.map((inp) => {
          ...
          >
            ...
            {showMethod ? <ChevronUp className="w-3.5 h-3.5 ml-auto" /> : <ChevronDown className="w-3.5 h-3.5 ml-auto" />}
          </button>
          <AnimatePresence>
            {showMethod && (
              <motion.div
              ...
              >
                {equations.length > 0 && (
                  <div>
                    <h4 className="text-[10px] font-bold text-white/40 uppercase tracking-wider mb-1.5">Equations</h4>
                    {equations.map((eq, i) => (
                      <p key={i} className="text-xs text-cyan-300/80 font-mono bg-cyan-500/5 px-2 py-1 rounded mb-1">{eq}</p>
                    ))}
                  </div>
                )}
                {assumptions.length > 0 && (
                  <div>
                    <h4 className="text-[10px] font-bold text-white/40 uppercase tracking-wider mb-1.5">Assumptions</h4>
                    {assumptions.map((a, i) => (
                ...
                {references && references.length > 0 && (
                  <div>
                    <h4 className="text-[10px] font-bold text-white/40 uppercase tracking-wider mb-1.5">References</h4>
                    {references.map((r, i) => (
                      <p key={i} className="text-[11px] text-white/30 italic">{r}</p>
                    ))}
                  </div>
                )}
          ...
          <AnimatePresence mode="wait">
            ...
            {calcState === 'error' && !results && (
              ...
              >
                ...
                <p className="text-sm text-red-300">{interpretation || 'Invalid inputs. Check highlighted fields.'}</p>
              </motion.div>
            )}
              ...
              >
                ...
                <div className="bg-[#0A1020] border border-white/8 rounded-lg p-4">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <button onClick={handleCopyResults} className="text-white/30 hover:text-white/60 transition-colors" title="Copy results">
                    ...
                    {results.map((r, i) => (
                      <div
                        ...
                        className={`flex items-center justify-between py-2 px-3 rounded-lg ${
                          r.highlight ? 'bg-cyan-500/8 border border-cyan-500/15' : 'bg-white/[0.02]'
                        }`}
                      >
                        <span className="text-xs text-white/60">{r.label}</span>
                        <span className={`text-sm font-mono font-bold ${r.highlight ? 'text-cyan-300' : 'text-white/90'}`}>
                          {typeof r.value === 'number' ? (Math.abs(r.value) > 1e6 || (Math.abs(r.value) < 0.01 && r.value !== 0) ? r.value.toExponential(4) : r.value.toFixed(4)) : r.value}
                ...
                {interpretation && (
                  <div className="bg-[#0A1020] border border-white/8 rounded-lg p-4">
                    <h3 className="text-xs font-bold text-white/60 uppercase tracking-wider mb-2">Interpretation</h3>
                ...
                {renderVisualization && chartData && (
                  <div className="bg-[#0A1020] border border-white/8 rounded-lg p-4">
                    <h3 className="text-xs font-bold text-white/60 uppercase tracking-wider mb-3">Visualization</h3>
