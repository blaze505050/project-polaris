...
export type SolverStatus =
  "RUNNING" | "CONVERGED" | "COMPLETED" | "WARNING" | "FAILED" | "OUT_OF_RANGE" | "APPROXIMATION";
...
export default function EngineeringStateBadge({
  ...
  const statusConfig = {
    ...
    WARNING: {
      label: "MODEL BOUNDARY WARNING",
      color: "text-amber-400 border-amber-500/30 bg-amber-500/10",
      icon: AlertTriangle,
      animate: false,
    },
    FAILED: {
      label: "SOLVER CONVERGENCE FAIL",
      color: "text-red-400 border-red-500/30 bg-red-500/10",
      icon: XCircle,
      animate: false,
    },
    OUT_OF_RANGE: {
      label: "OUT OF MODEL BOUNDS",
      color: "text-red-400 border-red-500/30 bg-red-500/10",
      icon: AlertTriangle,
      animate: false,
    },
    APPROXIMATION: {
      label: "PHYSICAL APPROXIMATION",
  ...
  return (
    <div
      className={`flex flex-wrap items-center justify-between gap-2 px-3 py-1.5 rounded-lg border font-mono text-xs ${statusConfig.color} ${className}`}
    >
      ...
      <div className="flex items-center gap-3 text-[10px] opacity-80">
        <span>
          METHOD: <strong className="text-white">{solverName}</strong>
        </span>
        {timeStep && (
          <span>
            Δt: <strong className="text-white">{timeStep}</strong>
          </span>
        )}
        {tolerance && (
          <span>
            TOL: <strong className="text-white">{tolerance}</strong>
          </span>
        )}
      </div>
    </div>
