...
export default function ExperimentHistoryLogger({
  ...
  const handleExportCSV = () => {
    const dataToExport = history.length ? history : [{
    ...
    link.setAttribute("download", `${moduleName.toLowerCase().replace(/\s+/g, '_')}_experiment_data.csv`);
  ...
  const handleExportJSON = () => {
    ...
    link.setAttribute("download", `${moduleName.toLowerCase().replace(/\s+/g, '_')}_experiment_data.json`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  };
  ...
  return (
    <div className={`rounded-xl border border-white/10 bg-[#080d1a] p-4 text-xs font-mono text-white/80 ${className}`}>
      ...
      {history.length > 0 ? (
        <div className="space-y-2 max-h-40 overflow-y-auto pr-1">
          {history.slice(-5).reverse().map((exp) => (
            <div key={exp.id} className="p-2 rounded bg-black/30 border border-white/5 flex items-center justify-between text-[11px]">
              <div>
                <span className="font-bold text-amber-400">{exp.id}</span>
                <span className="text-white/40 ml-2">{new Date(exp.timestamp).toLocaleTimeString()}</span>
              </div>
              <div className="text-white/60 truncate max-w-xs">
                Params: {Object.keys(exp.parameters || {}).length} | Results: {Object.keys(exp.results || {}).length}
              </div>
            </div>
          ))}
        </div>
      ) : (
        <p className="text-white/30 text-[11px] italic">No saved experiment runs for this session. Click "Log Experiment" to save your current inputs and outputs.</p>
      )}
    </div>
  );
}
