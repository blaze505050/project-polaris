...
import { ArrowLeft, Wind, Rocket, Cloud, Sliders, Gauge, Target, Crosshair, Zap, Globe, Ruler, BarChart3, Flame, Atom, Navigation } from 'lucide-react';
...
type ToolId =
  | 'airfoil' | 'reynolds' | 'mach' | 'lift' | 'drag'
  | 'dynamic-pressure' | 'ld-ratio' | 'stall-speed' | 'range' | 'endurance'
  | 'atmosphere' | 'wing-loading' | 'aspect-ratio' | 'propeller' | 'jet-thrust'
  | 'delta-v' | 'rocket-thrust' | 'orbital-velocity' | 'escape-velocity' | 'projectile';

const TOOLS: { id: ToolId; icon: React.ElementType; label: string; status: FeatureStatus; group: string }[] = [
  // Aerodynamics
  { id: 'airfoil', icon: Wind, label: 'NACA Airfoil Generator', status: 'available', group: 'Aerodynamics' },
  { id: 'reynolds', icon: Gauge, label: 'Reynolds Number', status: 'available', group: 'Aerodynamics' },
  { id: 'mach', icon: Zap, label: 'Mach Number', status: 'available', group: 'Aerodynamics' },
  { id: 'lift', icon: ArrowLeft, label: 'Lift Calculator', status: 'available', group: 'Aerodynamics' },
  { id: 'drag', icon: Sliders, label: 'Drag Calculator', status: 'available', group: 'Aerodynamics' },
  { id: 'dynamic-pressure', icon: Gauge, label: 'Dynamic Pressure', status: 'available', group: 'Aerodynamics' },
  { id: 'ld-ratio', icon: BarChart3, label: 'Lift-to-Drag Analyzer', status: 'available', group: 'Aerodynamics' },
  // Performance
  { id: 'stall-speed', icon: Target, label: 'Stall Speed', status: 'available', group: 'Performance' },
  { id: 'range', icon: Navigation, label: 'Aircraft Range', status: 'available', group: 'Performance' },
  { id: 'endurance', icon: Crosshair, label: 'Aircraft Endurance', status: 'available', group: 'Performance' },
  { id: 'atmosphere', icon: Cloud, label: 'ISA Atmosphere', status: 'available', group: 'Performance' },
  { id: 'wing-loading', icon: Ruler, label: 'Wing Loading', status: 'available', group: 'Performance' },
  { id: 'aspect-ratio', icon: Ruler, label: 'Aspect Ratio', status: 'available', group: 'Performance' },
  // Propulsion & Rocketry
  { id: 'propeller', icon: Wind, label: 'Propeller Performance', status: 'available', group: 'Propulsion' },
  { id: 'jet-thrust', icon: Flame, label: 'Jet Engine Thrust', status: 'available', group: 'Propulsion' },
  { id: 'delta-v', icon: Rocket, label: 'Rocket Delta-V', status: 'available', group: 'Propulsion' },
  { id: 'rocket-thrust', icon: Rocket, label: 'Rocket Thrust', status: 'available', group: 'Propulsion' },
  // Orbital & Trajectory
  { id: 'orbital-velocity', icon: Globe, label: 'Orbital Velocity', status: 'available', group: 'Orbital' },
  { id: 'escape-velocity', icon: Atom, label: 'Escape Velocity', status: 'available', group: 'Orbital' },
  { id: 'projectile', icon: Target, label: 'Projectile Trajectory', status: 'available', group: 'Orbital' },
];
      ...
      <div className="max-w-[120rem] mx-auto px-4 md:px-[4%] py-6">
        ...
        <div className="flex items-center gap-4 mb-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-white/40 font-mono">20 Working Tools · Aerodynamics · Performance · Propulsion · Orbital Mechanics</p>
          </div>
        </div>
...
function ToolRenderer({ toolId }: { toolId: ToolId }) {
  switch (toolId) {
    case 'airfoil': return <AirfoilTool />;
    case 'reynolds': return <ReynoldsTool />;
    case 'mach': return <MachTool />;
    case 'lift': return <LiftTool />;
    case 'drag': return <DragTool />;
    case 'dynamic-pressure': return <DynamicPressureTool />;
    case 'ld-ratio': return <LDRatioTool />;
    case 'stall-speed': return <StallSpeedTool />;
    case 'range': return <RangeTool />;
    case 'endurance': return <EnduranceTool />;
    case 'atmosphere': return <AtmosphereTool />;
    case 'wing-loading': return <WingLoadingTool />;
    case 'aspect-ratio': return <AspectRatioTool />;
    case 'propeller': return <PropellerTool />;
    case 'jet-thrust': return <JetThrustTool />;
    case 'delta-v': return <DeltaVTool />;
    case 'rocket-thrust': return <RocketThrustTool />;
    case 'orbital-velocity': return <OrbitalVelocityTool />;
    case 'escape-velocity': return <EscapeVelocityTool />;
...
function AirfoilTool() {
  ...
  useEffect(() => {
    ...
    const drawW = w - 2 * padX, drawH = h - 2 * padY;
    ...
    ctx.setLineDash([4, 4]);
    ctx.beginPath(); ctx.moveTo(toX(0), toY(0)); ctx.lineTo(toX(1), toY(0)); ctx.stroke();
    ctx.setLineDash([]);
    ...
    ctx.lineWidth = 1;
    ctx.beginPath();
    airfoil.camberLine.forEach((p, i) => i === 0 ? ctx.moveTo(toX(p.x), toY(p.y)) : ctx.lineTo(toX(p.x), toY(p.y)));
    ctx.stroke();
    ...
    ctx.lineWidth = 2;
    ctx.beginPath();
    airfoil.upper.forEach((p, i) => i === 0 ? ctx.moveTo(toX(p.x), toY(p.y)) : ctx.lineTo(toX(p.x), toY(p.y)));
    ctx.stroke();

    // Lower surface
    ctx.strokeStyle = '#06B6D4';
    ctx.beginPath();
    airfoil.lower.forEach((p, i) => i === 0 ? ctx.moveTo(toX(p.x), toY(p.y)) : ctx.lineTo(toX(p.x), toY(p.y)));
    ctx.stroke();
    ...
    ctx.fillText(`Max camber: ${(airfoil.maxCamber * 100).toFixed(1)}%`, padX + drawW - 120, padY + 14);
    ctx.fillText(`Max thickness: ${(airfoil.maxThickness * 100).toFixed(1)}%`, padX + drawW - 120, padY + 26);
  }, [airfoil, naca, chord]);
  ...
  const handleExportCSV = () => {
    ...
    for (let i = 0; i < airfoil.upper.length; i++) {
      rows.push(`${(airfoil.upper[i].x * chord).toFixed(6)},${(airfoil.upper[i].y * chord).toFixed(6)},${(airfoil.lower[i].y * chord).toFixed(6)}`);
    }
    const blob = new Blob([rows.join('\n')], { type: 'text/csv' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = `NACA${naca}_chord${chord}m.csv`; a.click();
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-xs text-white/50">Generate NACA 4-digit airfoil geometry with coordinate export</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="space-y-3">
          <div className="bg-[#0A1020] border border-white/8 rounded-lg p-4 space-y-3">
            ...
            <div>
              <label className="text-xs text-white/70 mb-1 block">NACA Designation (4 digits)</label>
              <input type="text" value={naca} onChange={(e) => setNaca(e.target.value.replace(/\D/g, '').slice(0, 4))}
                className="w-full bg-[#060B18] border border-white/10 rounded-lg px-3 py-2 text-sm text-white font-mono outline-none focus:border-cyan-500/50" maxLength={4} />
            </div>
            <div>
              <label className="text-xs text-white/70 mb-1 block">Chord Length <span className="text-white/30">m</span></label>
              <input type="number" value={chord} onChange={(e) => setChord(Math.max(0.01, parseFloat(e.target.value) || 1))} step={0.1}
            ...
            <div>
              <label className="text-xs text-white/70 mb-1 block">Resolution (points per surface)</label>
              <input type="number" value={resolution} onChange={(e) => setResolution(Math.max(20, Math.min(500, parseInt(e.target.value) || 100)))}
                className="w-full bg-[#060B18] border border-white/10 rounded-lg px-3 py-2 text-sm text-white font-mono outline-none focus:border-cyan-500/50" />
            </div>
          </div>
          {airfoil && (
            <div className="bg-[#0A1020] border border-white/8 rounded-lg p-4 space-y-2">
              <h3 className="text-xs font-bold text-white/60 uppercase tracking-wider">Properties</h3>
              {[
                { label: 'Max Camber', value: `${(airfoil.maxCamber * 100).toFixed(1)}%`, unit: 'chord' },
                { label: 'Camber Position', value: `${(airfoil.maxCamberPos * 100).toFixed(0)}%`, unit: 'chord' },
                { label: 'Max Thickness', value: `${(airfoil.maxThickness * 100).toFixed(1)}%`, unit: 'chord' },
                { label: 'α₀ (zero-lift)', value: `${airfoil.alphaZeroLift.toFixed(2)}°`, unit: '' },
                { label: 'Cl_α (lift slope)', value: `${airfoil.clAlpha.toFixed(3)}`, unit: '/rad' },
              ].map((r, i) => (
                <div key={i} className="flex justify-between py-1.5 px-2 rounded bg-white/[0.02]">
                  ...
                  <span className="text-xs font-mono text-white/90">{r.value} <span className="text-white/30">{r.unit}</span></span>
                </div>
              ))}
            </div>
          )}
          <button onClick={handleExportCSV} disabled={!airfoil}
        ...
        <div className="bg-[#0A1020] border border-white/8 rounded-lg overflow-hidden">
          ...
          {!airfoil && <p className="text-center text-xs text-red-400 py-4">Invalid NACA designation. Enter exactly 4 digits.</p>}
        </div>
      </div>
    </div>
  );
  ...
  return (
    <ToolShell
      ...
      inputs={[
        ...
        { key: 'chord', label: 'Characteristic Length', unit: 'm', defaultValue: 1.5, min: 0.001, max: 100, helpText: 'Chord, diameter, or length' },
        { key: 'density', label: 'Air Density', unit: 'kg/m³', defaultValue: 1.225, min: 0.001, max: 100 },
        { key: 'viscosity', label: 'Dynamic Viscosity', unit: 'Pa·s', defaultValue: 1.789e-5, min: 1e-7, max: 1, step: 1e-6 },
      ]}
      equations={['Re = ρVL / μ']}
      assumptions={['Newtonian fluid', 'Incompressible flow at low Mach numbers', 'External flow thresholds: Laminar < 5×10⁵ < Transitional < 10⁶ < Turbulent']}
...
function MachTool() {
  return (
    <ToolShell
      ...
      inputs={[
        { key: 'velocity', label: 'Flight Velocity', unit: 'm/s', defaultValue: 250, min: 0, max: 100000 },
        { key: 'speedOfSound', label: 'Speed of Sound', unit: 'm/s', defaultValue: 340.3, min: 1, max: 2000, helpText: 'At sea level: ~340.3 m/s' },
      ]}
      equations={['M = V / a']}
  ...
  return (
    <ToolShell
      ...
      inputs={[
        ...
        { key: 'cl', label: 'Lift Coefficient (CL)', unit: '—', defaultValue: 0.5, min: -3, max: 5, step: 0.01 },
      ]}
      equations={['L = ½ρV²SCL']}
      assumptions={['Steady, level flight', 'Uniform flow conditions', 'CL assumed constant across span']}
      onCalculate={(v) => {
  ...
  return (
    <ToolShell
      ...
      inputs={[
        { key: 'density', label: 'Air Density', unit: 'kg/m³', defaultValue: 1.225, min: 0.001, max: 100 },
        { key: 'velocity', label: 'Velocity', unit: 'm/s', defaultValue: 70, min: 0, max: 10000 },
        { key: 'area', label: 'Reference Area', unit: 'm²', defaultValue: 16, min: 0.01, max: 1000 },
        { key: 'cd', label: 'Drag Coefficient (CD)', unit: '—', defaultValue: 0.03, min: 0, max: 5, step: 0.001 },
      ]}
      equations={['D = ½ρV²SCD']}
      assumptions={['Total drag coefficient includes parasite + induced drag', 'Constant CD assumed']}
      onCalculate={(v) => {
...
function DynamicPressureTool() {
  return (
    <ToolShell
      ...
      inputs={[
        { key: 'density', label: 'Fluid Density', unit: 'kg/m³', defaultValue: 1.225, min: 0.001, max: 1000 },
...
function LDRatioTool() {
  return (
    <ToolShell
      ...
      inputs={[
        { key: 'cl', label: 'Lift Coefficient (CL)', unit: '—', defaultValue: 0.5, min: -3, max: 5, step: 0.01 },
        { key: 'cd', label: 'Drag Coefficient (CD)', unit: '—', defaultValue: 0.03, min: 0.0001, max: 5, step: 0.001 },
      ]}
      equations={['L/D = CL / CD']}
      assumptions={['Coefficients at a single operating point', 'Does not account for CL/CD variation with angle of attack']}
      onCalculate={(v) => {
...
function StallSpeedTool() {
  return (
    <ToolShell
      ...
      inputs={[
        { key: 'mass', label: 'Aircraft Mass', unit: 'kg', defaultValue: 1500, min: 0.1, max: 1000000 },
        { key: 'wingArea', label: 'Wing Area', unit: 'm²', defaultValue: 16, min: 0.1, max: 1000 },
        { key: 'clMax', label: 'Max Lift Coefficient (CL_max)', unit: '—', defaultValue: 1.6, min: 0.1, max: 5, step: 0.1, helpText: 'Typically 1.2-2.5 depending on high-lift devices' },
        { key: 'density', label: 'Air Density', unit: 'kg/m³', defaultValue: 1.225, min: 0.001, max: 100 },
      ]}
...
function RangeTool() {
  return (
    <ToolShell
      ...
      inputs={[
        { key: 'liftToDrag', label: 'Lift-to-Drag Ratio (L/D)', unit: '—', defaultValue: 15, min: 1, max: 80, step: 0.5 },
        { key: 'initialWeight', label: 'Initial Weight', unit: 'N', defaultValue: 200000, min: 1, max: 1e8, helpText: 'Takeoff weight including fuel' },
        { key: 'finalWeight', label: 'Final Weight', unit: 'N', defaultValue: 140000, min: 1, max: 1e8, helpText: 'Landing weight (initial minus fuel burned)' },
        { key: 'velocity', label: 'Cruise Velocity', unit: 'm/s', defaultValue: 230, min: 1, max: 10000 },
        { key: 'sfc', label: 'Specific Fuel Consumption', unit: 'kg/(N·s)', defaultValue: 1.7e-5, min: 1e-7, max: 1, step: 1e-6, helpText: 'Typical jet SFC: 1.5-2.0 × 10⁻⁵ kg/(N·s)' },
      ]}
      equations={['R = (V / SFC) · (L/D) · ln(W₀/W₁)']}
      assumptions={['Breguet range equation for jet propulsion', 'Constant velocity, L/D, and SFC throughout cruise', 'No wind effects', 'Steady, level flight']}
      onCalculate={(v) => {
        const r = computeAircraftRange('jet', v.liftToDrag, v.initialWeight, v.finalWeight, v.velocity, v.sfc);
        if (!r) return null;
        return {
          results: [
...
function EnduranceTool() {
  return (
    <ToolShell
      ...
      domain="Aircraft Performance · Mission Planning"
      inputs={[
        { key: 'liftToDrag', label: 'Lift-to-Drag Ratio (L/D)', unit: '—', defaultValue: 18, min: 1, max: 80 },
        { key: 'initialWeight', label: 'Initial Weight', unit: 'N', defaultValue: 200000, min: 1, max: 1e8 },
        { key: 'finalWeight', label: 'Final Weight', unit: 'N', defaultValue: 150000, min: 1, max: 1e8 },
        { key: 'sfc', label: 'Specific Fuel Consumption', unit: 'kg/(N·s)', defaultValue: 1.7e-5, min: 1e-7, max: 1, step: 1e-6 },
      ]}
      equations={['E = (1/SFC) · (L/D) · ln(W₀/W₁)']}
      assumptions={['Breguet endurance equation for jet propulsion', 'Constant L/D and SFC', 'Minimum drag speed for max endurance']}
      onCalculate={(v) => {
        const r = computeAircraftEndurance('jet', v.liftToDrag, v.initialWeight, v.finalWeight, v.sfc);
        if (!r) return null;
        return {
          results: [
...
function AtmosphereTool() {
  return (
    <ToolShell
      ...
      inputs={[
        { key: 'altitude', label: 'Altitude', unit: 'm', defaultValue: 10000, min: 0, max: 84852, helpText: 'Valid: 0 – 84,852 m' },
      ]}
      equations={[
      ...
      assumptions={['US Standard Atmosphere 1976 / ISO 2533', 'Hydrostatic equilibrium', 'Ideal gas law', 'No winds or weather']}
...
function WingLoadingTool() {
  return (
    <ToolShell
      ...
      inputs={[
        { key: 'weight', label: 'Aircraft Weight', unit: 'N', defaultValue: 50000, min: 1, max: 1e8, helpText: 'W = m × g₀' },
  ...
  return (
    <ToolShell
      ...
      inputs={[
        ...
        { key: 'wingArea', label: 'Wing Area', unit: 'm²', defaultValue: 16, min: 0.01, max: 10000 },
      ]}
      equations={['AR = b² / S']}
...
function PropellerTool() {
  return (
    <ToolShell
      ...
      status="beta"
      inputs={[
        { key: 'diameter', label: 'Propeller Diameter', unit: 'm', defaultValue: 1.8, min: 0.1, max: 10 },
        { key: 'rpm', label: 'Rotational Speed', unit: 'RPM', defaultValue: 2400, min: 1, max: 50000 },
        { key: 'airspeed', label: 'Freestream Airspeed', unit: 'm/s', defaultValue: 50, min: 0, max: 300 },
        { key: 'density', label: 'Air Density', unit: 'kg/m³', defaultValue: 1.225, min: 0.001, max: 10 },
        { key: 'ct', label: 'Thrust Coefficient (CT)', unit: '—', defaultValue: 0.05, min: 0, max: 0.5, step: 0.005 },
        { key: 'cp', label: 'Power Coefficient (CP)', unit: '—', defaultValue: 0.04, min: 0.001, max: 0.5, step: 0.005 },
      ]}
      equations={['T = CT · ρ · n² · D⁴', 'P = CP · ρ · n³ · D⁵', 'η = J · CT / CP', 'J = V / (nD)']}
      assumptions={['CT and CP assumed constant (simplified)', 'No compressibility correction', 'Momentum/blade-element model approximation']}
      onCalculate={(v) => {
...
function JetThrustTool() {
  return (
    <ToolShell
      ...
      inputs={[
        { key: 'massFlowRate', label: 'Mass Flow Rate', unit: 'kg/s', defaultValue: 100, min: 0.1, max: 1000 },
        { key: 'exitVelocity', label: 'Exit (Exhaust) Velocity', unit: 'm/s', defaultValue: 500, min: 1, max: 5000 },
        { key: 'flightVelocity', label: 'Flight Velocity', unit: 'm/s', defaultValue: 230, min: 0, max: 5000 },
        { key: 'exitPressure', label: 'Nozzle Exit Pressure', unit: 'Pa', defaultValue: 101325, min: 0, max: 1e7 },
        { key: 'ambientPressure', label: 'Ambient Pressure', unit: 'Pa', defaultValue: 101325, min: 0, max: 200000 },
        { key: 'exitArea', label: 'Nozzle Exit Area', unit: 'm²', defaultValue: 0.5, min: 0.01, max: 10 },
      ]}
      equations={['F = ṁ(Ve - V₀) + (Pe - Pa)Ae']}
      assumptions={['Steady-state, 1D flow', 'No installation losses', 'Uniform exit conditions', 'No bleed or power extraction']}
      onCalculate={(v) => {
        const r = computeJetEngineThrust(v.massFlowRate, v.exitVelocity, v.flightVelocity, v.exitPressure, v.ambientPressure, v.exitArea);
        if (!r) return null;
        return {
          results: [
...
function DeltaVTool() {
  return (
    <ToolShell
      ...
      inputs={[
        { key: 'initialMass', label: 'Initial (Wet) Mass', unit: 'kg', defaultValue: 100000, min: 0.1, max: 1e9 },
        { key: 'finalMass', label: 'Final (Dry) Mass', unit: 'kg', defaultValue: 10000, min: 0.1, max: 1e9, helpText: 'Must be less than initial mass' },
        { key: 'isp', label: 'Specific Impulse (Isp)', unit: 's', defaultValue: 311, min: 1, max: 10000, helpText: 'LOX/RP-1: ~311s, LOX/LH2: ~450s' },
      ]}
      equations={['ΔV = Isp · g₀ · ln(m₀/mf)', 'Ve = Isp · g₀']}
      assumptions={['Ideal, no gravity losses', 'No atmospheric drag', 'Constant Isp throughout burn', 'Single stage']}
...
function RocketThrustTool() {
  return (
    <ToolShell
      ...
      inputs={[
        { key: 'massFlowRate', label: 'Mass Flow Rate', unit: 'kg/s', defaultValue: 250, min: 0.01, max: 10000 },
        { key: 'exhaustVelocity', label: 'Exhaust Velocity', unit: 'm/s', defaultValue: 3000, min: 1, max: 50000 },
        { key: 'exitPressure', label: 'Exit Pressure', unit: 'Pa', defaultValue: 101325, min: 0, max: 1e8 },
        { key: 'ambientPressure', label: 'Ambient Pressure', unit: 'Pa', defaultValue: 101325, min: 0, max: 200000 },
        { key: 'exitArea', label: 'Nozzle Exit Area', unit: 'm²', defaultValue: 1.0, min: 0.001, max: 100 },
      ]}
      equations={['F = ṁ·Ve + (Pe - Pa)·Ae', 'Isp = Ve / g₀']}
      assumptions={['Steady-state operation', 'Quasi-1D nozzle flow', 'Uniform exit conditions']}
      onCalculate={(v) => {
        const r = computeRocketThrust(v.massFlowRate, v.exhaustVelocity, v.exitPressure, v.ambientPressure, v.exitArea);
        if (!r) return null;
        return {
          results: [
            { label: 'Thrust', value: r.thrust, unit: 'N', highlight: true },
...
function OrbitalVelocityTool() {
  return (
    <ToolShell
      ...
      inputs={[
        { key: 'altitude', label: 'Orbital Altitude', unit: 'km', defaultValue: 400, min: 0, max: 1e9, helpText: 'Above surface (e.g., ISS ≈ 408 km)' },
        { key: 'bodyRadius', label: 'Central Body Radius', unit: 'km', defaultValue: 6371, min: 1, max: 1e9, helpText: 'Earth = 6371 km' },
        { key: 'bodyMass', label: 'Central Body Mass', unit: 'kg', defaultValue: 5.972e24, min: 1, max: 1e31, helpText: 'Earth = 5.972×10²⁴ kg' },
      ]}
      equations={['v = √(μ/r)', 'μ = G·M', 'r = R_body + altitude']}
      assumptions={['Circular orbit', 'Point mass central body', 'No perturbations (J2, drag, etc.)']}
      onCalculate={(v) => {
...
function EscapeVelocityTool() {
  return (
    <ToolShell
      ...
      domain="Orbital Mechanics · Astrodynamics"
      inputs={[
        { key: 'altitude', label: 'Launch Altitude', unit: 'km', defaultValue: 0, min: 0, max: 1e9, helpText: 'Above surface' },
        { key: 'bodyRadius', label: 'Body Radius', unit: 'km', defaultValue: 6371, min: 1, max: 1e9 },
        { key: 'bodyMass', label: 'Body Mass', unit: 'kg', defaultValue: 5.972e24, min: 1, max: 1e31 },
      ]}
      equations={['v_esc = √(2μ/r)', 'μ = GM']}
      assumptions={['Point mass central body', 'No atmospheric drag', 'No third-body perturbations']}
      onCalculate={(v) => {
        const G = PHYSICS_CONSTANTS.G;
        const mu = G * v.bodyMass;
...
function ProjectileTool() {
  ...
  return (
    <ToolShell
      ...
      inputs={[
        { key: 'velocity', label: 'Launch Velocity', unit: 'm/s', defaultValue: 100, min: 0.1, max: 100000 },
        { key: 'angle', label: 'Launch Angle', unit: '°', defaultValue: 45, min: 0, max: 90, step: 1 },
        { key: 'height', label: 'Initial Height', unit: 'm', defaultValue: 0, min: 0, max: 100000 },
      ]}
      equations={['x(t) = V₀·cos(θ)·t', 'y(t) = h₀ + V₀·sin(θ)·t - ½gt²']}
      assumptions={['No air resistance (vacuum trajectory)', 'Flat Earth approximation', 'Constant g = 9.80665 m/s²', 'Point mass']}
      onCalculate={(v) => {
