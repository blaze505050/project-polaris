...
export default function PhotometrySuite() {
  ...
  const photResult = useMemo(() => {
    ...
    return computeAperturePhotometry(
      imageData, aperturePos.x, aperturePos.y,
      apertureRadius, innerAnnulus, outerAnnulus,
      readNoise, 0.01, exposureTime, gain
    );
  }, [aperturePos, apertureRadius, innerAnnulus, outerAnnulus, imageData, readNoise, exposureTime, gain]);

  // Color mapping functions
  const applyColorMap = useCallback((value: number, min: number, max: number): [number, number, number] => {
    const t = Math.max(0, Math.min(1, (value - min) / (max - min + 1)));
    switch (colorMap) {
      case 'heat':
        return [Math.min(255, t * 510), Math.min(255, Math.max(0, (t - 0.33) * 760)), Math.min(255, Math.max(0, (t - 0.67) * 760))];
      case 'viridis':
  ...
  useEffect(() => {
    ...
    if (aperturePos) {
      ...
      ctx.lineWidth = 1;
      ctx.beginPath();
      ctx.moveTo(ax - 15, ay); ctx.lineTo(ax + 15, ay);
      ctx.moveTo(ax, ay - 15); ctx.lineTo(ax, ay + 15);
      ctx.stroke();
    }
  }, [imageData, aperturePos, apertureRadius, innerAnnulus, outerAnnulus, applyColorMap, imgWidth, imgHeight]);
      ...
      <div className="max-w-[120rem] mx-auto px-4 md:px-[4%] py-6">
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-white/50 font-mono">Click any star to measure Net Sky-Subtracted Flux & SNR</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2">
            <div className="rounded-xl overflow-hidden border border-white/10 bg-black"
              ...
              <canvas ref={canvasRef} className="w-full cursor-crosshair" style={{ height: '500px' }}
              ...
              <div className="flex items-center gap-2 px-4 py-3 bg-white/[0.02] border-t border-white/5">
                ...
                <button onClick={() => setAperturePos(null)} className="px-3 py-1 text-xs font-mono text-white/40 hover:text-white/70 flex items-center gap-1">
            ...
            {userMode === 'student' && (
              <div className="mt-4 p-4 rounded-xl bg-amber-500/5 border border-amber-500/10">
                <p className="text-sm text-amber-400/80">
                  💡 <strong>How Aperture Photometry Works:</strong> Click on a star to place a circular aperture (cyan circle). The sky background is measured in the annular ring (yellow dashed). Net flux = Star counts − Sky counts. Signal-to-Noise Ratio (SNR) tells you how confident the measurement is.
                </p>
              </div>
            )}
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-xl bg-white/[0.03] border border-white/10 p-4" style={{ backdropFilter: 'blur(20px)' }}>
              ...
              {[
                { label: 'Aperture Radius', value: apertureRadius, set: setApertureRadius, min: 3, max: 25, unit: 'px' },
                { label: 'Inner Annulus', value: innerAnnulus, set: setInnerAnnulus, min: 5, max: 35, unit: 'px' },
                { label: 'Outer Annulus', value: outerAnnulus, set: setOuterAnnulus, min: 10, max: 50, unit: 'px' },
              ].map((param) => (
                <div key={param.label} className="mb-3">
                  <div className="flex justify-between text-xs mb-1">
                    <span className="text-white/50">{param.label}</span>
                    <span className="font-mono text-amber-400">{param.value} {param.unit}</span>
                  </div>
              ...
              {userMode === 'professional' && (
                <>
                  <div className="border-t border-white/5 my-3 pt-3">
                    ...
                    {[
                      { label: 'Read Noise (e⁻/pix)', value: readNoise, set: setReadNoise, min: 1, max: 20 },
                      { label: 'Gain (e⁻/ADU)', value: gain, set: (v: number) => setGain(v / 10), min: 5, max: 50, display: gain.toFixed(1) },
                      { label: 'Exposure (s)', value: exposureTime, set: setExposureTime, min: 1, max: 3600 },
                      { label: 'Sky Background (ADU)', value: skyBackground, set: setSkyBackground, min: 50, max: 500 },
                    ].map((param) => (
                      <div key={param.label} className="mb-2">
                        <div className="flex justify-between text-xs mb-1">
                          <span className="text-white/40">{param.label}</span>
                          <span className="font-mono text-white/60">{'display' in param ? (param as any).display : param.value}</span>
                        </div>
                        <input type="range" min={param.min} max={param.max} value={param.value}
                          onChange={(e) => param.set(Number(e.target.value))}
            ...
            {photResult && aperturePos && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                ...
                <div className="space-y-1.5">
                  {[
                    { label: 'Position', value: `(${aperturePos.x}, ${aperturePos.y})`, highlight: false },
                    { label: 'Source Flux', value: `${photResult.sourceFlux.toFixed(0)} ADU`, highlight: false },
                    { label: 'Sky/pixel', value: `${photResult.skyFluxPerPixel.toFixed(1)} ADU`, highlight: false },
                    { label: 'Net Flux', value: `${photResult.netFlux.toFixed(1)} ADU`, highlight: true },
                    { label: 'SNR', value: photResult.snr.toFixed(2), highlight: true },
                    { label: 'Instrumental Mag', value: photResult.instrumentalMag < 50 ? photResult.instrumentalMag.toFixed(3) : 'N/A', highlight: true },
                    { label: 'Source Pixels', value: photResult.sourcePixels.toString(), highlight: false },
                    { label: 'Sky Pixels', value: photResult.skyPixels.toString(), highlight: false },
                  ].map((row) => (
                    <div key={row.label} className={`flex justify-between items-center py-1.5 border-b border-white/5 ${row.highlight ? '' : ''}`}>
                      <span className="text-xs text-white/40">{row.label}</span>
                      <span className={`font-mono text-sm ${row.highlight ? 'text-amber-400 font-bold' : 'text-white/80'}`}>{row.value}</span>
                    </div>
                  ))}
                </div>
                ...
                {userMode === 'professional' && (
                  <div className="mt-3 p-3 rounded-lg bg-black/30 border border-white/5">
                    <p className="text-xs font-mono text-white/40 leading-relaxed">
                      SNR = S_net / √(S_src + n·B_sky + n·D·t + n·R²)<br />
                      m_inst = -2.5 · log₁₀(F_net)<br />
                      n_src = {photResult.sourcePixels}, R² contrib = {photResult.readNoiseContrib.toFixed(1)} e⁻
                    </p>
                  </div>
                )}
              </motion.div>
            ...
            {!aperturePos && (
              <div className="rounded-xl bg-white/[0.03] border border-white/10 p-6 text-center" style={{ backdropFilter: 'blur(20px)' }}>
                <Crosshair className="w-8 h-8 mx-auto mb-3 text-amber-400/30" />
                <p className="text-sm text-white/40">Click on a star in the image to begin photometric measurement</p>
              </div>
            )}
          </div>
        </div>
