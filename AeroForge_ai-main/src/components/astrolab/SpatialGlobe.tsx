...
import { ArrowLeft, Play, Pause, RotateCcw, Satellite, Eye, EyeOff, Zap, Clock } from 'lucide-react';
...
export default function SpatialGlobe() {
  ...
  useEffect(() => {
    ...
    for (const [key, tle] of Object.entries(TLE_DATA)) {
      try {
        ...
        initSats.push({
          name: tle.name,
          lat: 0, lon: 0, alt: 400, velocity: 7.66,
          color: tle.color,
          trail: [],
          satrec,
        });
  ...
  const propagate = useCallback((date: Date) => {
    const updated = satsRef.current.map((sat) => {
      ...
      } catch { /* SGP4 propagation failure — keep last known position */ }
      return sat;
    });
  ...
  useEffect(() => {
    ...
    const project = (lat: number, lon: number): { x: number; y: number; visible: boolean } => {
      ...
      const py = Math.sin(latR) * Math.cos(rotation.phi) - Math.cos(latR) * Math.cos(lonR) * Math.sin(rotation.phi);
      const pz = Math.sin(latR) * Math.sin(rotation.phi) + Math.cos(latR) * Math.cos(lonR) * Math.cos(rotation.phi);
    ...
    const continents: Array<Array<[number, number]>> = [
      ...
      [[-10,75],[0,70],[10,65],[15,55],[25,50],[30,45],[35,30],[30,25],[25,30],[15,35],[5,40],[-5,50],[-15,60],[-10,75]].map(([lon,lat]) => [lat, lon-100]),
      // South America
      [[-10,-35],[-5,-55],[5,-60],[15,-55],[12,-35],[5,-15],[-5,-10],[-10,-35]].map(([lon,lat]) => [lat, lon-60]),
      // Europe
      [[40,0],[45,-5],[50,5],[55,15],[60,30],[55,40],[50,30],[45,20],[40,0]].map(([lat,lon]) => [lat, lon]),
      // Africa
      [[35,0],[30,-10],[15,-15],[5,-10],[0,10],[-5,20],[-15,35],[-30,30],[-35,20],[-25,15],[-10,0],[5,-5],[15,0],[25,5],[35,0]].map(([lat,lon]) => [lat, lon]),
      // Asia (simplified)
      [[40,30],[45,50],[50,60],[55,80],[60,100],[55,120],[50,130],[45,140],[40,130],[35,120],[30,100],[25,80],[30,60],[35,40],[40,30]].map(([lat,lon]) => [lat, lon]),
      // Australia
      [[-25,115],[-20,130],[-15,140],[-25,150],[-35,145],[-35,135],[-30,120],[-25,115]].map(([lat,lon]) => [lat, lon]),
    ];
      ...
      if (showTrails && sat.trail.length > 1) {
        for (let i = 1; i < sat.trail.length; i++) {
          ...
          if (p0.visible && p1.visible) {
            ctx.strokeStyle = sat.color.replace(')', `,${sat.trail[i].alpha * 0.6})`).replace('rgb', 'rgba').replace('#', '');
            // Convert hex to rgba
      ...
      <div className="max-w-[120rem] mx-auto px-4 md:px-[4%] py-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-white/50 font-mono">Real-time satellite tracking via Two-Line Element sets</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <Clock className="w-4 h-4 text-cyan-400" />
            <span className="font-mono text-sm text-cyan-400">{simTime.toISOString().replace('T', ' ').slice(0, 19)} UTC</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <div className="lg:col-span-3">
            <div className="relative rounded-xl overflow-hidden border border-white/10 bg-[#0a1020]"
              ...
              <div className="absolute bottom-4 left-4 flex gap-2">
                ...
                <button onClick={() => { setSimTime(new Date()); }}
                  className="p-2.5 rounded-lg bg-black/60 backdrop-blur-sm border border-white/10 hover:border-cyan-500/50 transition-all">
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-4">
            ...
            <div className="rounded-xl bg-white/[0.03] border border-white/10 p-4" style={{ backdropFilter: 'blur(20px)' }}>
              ...
              <div className="space-y-2">
                {satsRef.current.map((sat) => (
                  <button key={sat.name} onClick={() => setSelectedSat(sat.name)}
                    className={`w-full text-left px-3 py-2.5 rounded-lg border transition-all ${
                      selectedSat === sat.name ? 'border-cyan-500/40 bg-cyan-500/10' : 'border-white/5 bg-white/[0.02] hover:bg-white/[0.05]'
                    }`}>
                    <div className="flex items-center gap-2">
                      <div className="w-2.5 h-2.5 rounded-full animate-pulse" style={{ backgroundColor: sat.color }} />
            ...
            {currentSatData && (
              <div className="rounded-xl bg-white/[0.03] border border-white/10 p-4" style={{ backdropFilter: 'blur(20px)' }}>
                <h3 className="text-sm font-semibold mb-3 flex items-center gap-2" style={{ color: currentSatData.color }}>
                ...
                {userMode === 'student' && (
                  <div className="mt-3 p-2.5 rounded-lg bg-cyan-500/5 border border-cyan-500/10">
                    <p className="text-xs text-cyan-400/70">💡 Satellites in LEO orbit Earth every ~90 minutes at ~7.7 km/s. The ISS orbits at 51.6° inclination to cover most populated areas.</p>
                  </div>
                )}
              </div>
            )}
            ...
            <div className="rounded-xl bg-white/[0.03] border border-white/10 p-4" style={{ backdropFilter: 'blur(20px)' }}>
              ...
              <label className="flex items-center justify-between py-2 cursor-pointer">
                ...
                <button onClick={() => setShowGrid(!showGrid)}
                  ...
                  <div className={`w-4 h-4 rounded-full bg-white transition-transform ${showGrid ? 'translate-x-5' : 'translate-x-0.5'}`} />
                </button>
              </label>
              <label className="flex items-center justify-between py-2 cursor-pointer">
                ...
                <button onClick={() => setShowTrails(!showTrails)}
                  ...
                  <div className={`w-4 h-4 rounded-full bg-white transition-transform ${showTrails ? 'translate-x-5' : 'translate-x-0.5'}`} />
                </button>
              </label>
            </div>
          </motion.div>
