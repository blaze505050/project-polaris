...
const CONSTELLATIONS: ConstellationShell[] = [
  { name: 'Starlink', operator: 'SpaceX', regime: 'LEO', color: '#00F0FF', altitudeMin: 340, altitudeMax: 614, altitudeAvg: 550, activePayloads: 5600, inclination: 53, planes: 72, satsPerPlane: 22 },
  { name: 'OneWeb', operator: 'Eutelsat OneWeb', regime: 'LEO', color: '#06B6D4', altitudeMin: 1200, altitudeMax: 1200, altitudeAvg: 1200, activePayloads: 634, inclination: 87.9, planes: 18, satsPerPlane: 36 },
  { name: 'Iridium NEXT', operator: 'Iridium', regime: 'LEO', color: '#22D3EE', altitudeMin: 780, altitudeMax: 780, altitudeAvg: 780, activePayloads: 66, inclination: 86.4, planes: 6, satsPerPlane: 11 },
  { name: 'GPS', operator: 'US Space Force', regime: 'MEO', color: '#F59E0B', altitudeMin: 20180, altitudeMax: 20220, altitudeAvg: 20200, activePayloads: 31, inclination: 55, planes: 6, satsPerPlane: 4 },
  { name: 'Galileo', operator: 'ESA', regime: 'MEO', color: '#D97706', altitudeMin: 23222, altitudeMax: 23222, altitudeAvg: 23222, activePayloads: 28, inclination: 56, planes: 3, satsPerPlane: 10 },
  { name: 'GLONASS', operator: 'Roscosmos', regime: 'MEO', color: '#FBBF24', altitudeMin: 19100, altitudeMax: 19100, altitudeAvg: 19100, activePayloads: 24, inclination: 64.8, planes: 3, satsPerPlane: 8 },
  { name: 'Inmarsat', operator: 'Inmarsat/Viasat', regime: 'GEO', color: '#A78BFA', altitudeMin: 35786, altitudeMax: 35786, altitudeAvg: 35786, activePayloads: 14, inclination: 0, planes: 1, satsPerPlane: 14 },
  { name: 'SES', operator: 'SES S.A.', regime: 'GEO', color: '#8B5CF6', altitudeMin: 35786, altitudeMax: 35786, altitudeAvg: 35786, activePayloads: 50, inclination: 0, planes: 1, satsPerPlane: 50 },
  { name: 'TDRS', operator: 'NASA', regime: 'GEO', color: '#C084FC', altitudeMin: 35786, altitudeMax: 35786, altitudeAvg: 35786, activePayloads: 9, inclination: 0, planes: 1, satsPerPlane: 9 },
];
...
export default function Constellations() {
  ...
  // Stats
  const stats = useMemo(() => {
    const byRegime = { LEO: { count: 0, payloads: 0 }, MEO: { count: 0, payloads: 0 }, GEO: { count: 0, payloads: 0 } };
  ...
  const filteredConstellations = selectedRegime === 'ALL'
    ? CONSTELLATIONS
  ...
  useEffect(() => {
    ...
    filteredConstellations.forEach((c) => {
      ...
      ctx.textAlign = 'left';
      ctx.fillText(`${c.name} (${(c.altitudeAvg / 1000).toFixed(0)}k km)`, cx + r + 8, cy - r * 0.1);
    });
      ...
      <div className="max-w-[120rem] mx-auto px-4 md:px-[4%] py-6">
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-white/50 font-mono">Multi-shell LEO/MEO/GEO tracking · Active payload counts</p>
          </div>
        </div>

        {/* Regime Stats Bar */}
        <div className="grid grid-cols-4 gap-3 mb-6">
          <button onClick={() => setSelectedRegime('ALL')}
            ...
            <span className="block text-lg font-bold font-mono text-white/80 mt-1">{CONSTELLATIONS.reduce((s, c) => s + c.activePayloads, 0).toLocaleString()}</span>
          </button>
          {(Object.entries(REGIME_CONFIG) as [keyof typeof REGIME_CONFIG, typeof REGIME_CONFIG[keyof typeof REGIME_CONFIG]][]).map(([key, cfg]) => (
            <button key={key} onClick={() => setSelectedRegime(key)}
              ...
              style={selectedRegime === key ? { borderColor: cfg.color + '40', backgroundColor: cfg.color + '08' } : {}}>
              <Satellite className="w-5 h-5 mx-auto mb-1" style={{ color: cfg.color }} />
              <span className="text-xs" style={{ color: cfg.color + '80' }}>{key}</span>
              <span className="block text-lg font-bold font-mono mt-1" style={{ color: cfg.color }}>{stats[key].payloads.toLocaleString()}</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2">
            <div className="rounded-xl overflow-hidden border border-white/10" style={{ boxShadow: '0 0 40px rgba(0,0,0,0.3)' }}>
          ...
          <div className="space-y-3 max-h-[550px] overflow-y-auto pr-1">
            {filteredConstellations.map((c) => (
              <motion.button key={c.name} onClick={() => setSelectedConstellation(selectedConstellation === c.name ? null : c.name)}
                ...
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-xs font-mono px-2 py-0.5 rounded" style={{ backgroundColor: REGIME_CONFIG[c.regime].color + '15', color: REGIME_CONFIG[c.regime].color }}>
                    {c.regime}
                  </span>
                </div>
                  ...
                  {selectedConstellation === c.name && (
                    <>
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="text-white/70">{c.altitudeMin}–{c.altitudeMax} km</span>
                      </div>
                    </>
                  )}
                </div>
        ...
        {userMode === 'student' && (
          <div className="mt-6 p-4 rounded-xl bg-violet-500/5 border border-violet-500/10 max-w-3xl">
            <p className="text-sm text-violet-400/80">
              💡 <strong>Orbital Regimes:</strong> LEO satellites (cyan) orbit below 2,000 km for low-latency comms and Earth observation. MEO satellites (amber) at ~20,000 km provide navigation (GPS/Galileo). GEO satellites (violet) at 35,786 km appear stationary, ideal for broadcast TV and weather monitoring.
            </p>
          </div>
        )}
      </div>
