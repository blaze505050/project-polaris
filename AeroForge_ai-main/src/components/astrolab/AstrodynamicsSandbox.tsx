...
import { rk4NBodyStep, computeNBodyEnergy, type NBodyState, PHYSICS_CONSTANTS } from '@/services/physicsEngine';
...
const PRESETS: Record<string, { name: string; bodies: BodyConfig[] }> = {
  'sun-earth-moon': {
    ...
    bodies: [
      ...
      { name: 'Earth', x: 1.496e11, y: 0, vx: 0, vy: 29780, mass: 5.972e24, color: '#00F0FF', trail: [] },
      { name: 'Mars', x: 2.279e11, y: 0, vx: 0, vy: 24070, mass: 6.417e23, color: '#FF007A', trail: [] },
    ],
  },
  'binary-star': {
    name: 'Binary Star + Planet',
    bodies: [
      { name: 'Star A', x: -5e10, y: 0, vx: 0, vy: -15000, mass: 1.5e30, color: '#F59E0B', trail: [] },
      { name: 'Star B', x: 5e10, y: 0, vx: 0, vy: 15000, mass: 1.2e30, color: '#A78BFA', trail: [] },
  ...
  'figure-eight': {
    ...
    bodies: [
      { name: 'Body 1', x: -1e11, y: 0, vx: 0, vy: -20000, mass: 1e30, color: '#00F0FF', trail: [] },
      { name: 'Body 2', x: 1e11, y: 0, vx: 0, vy: 20000, mass: 1e30, color: '#FF007A', trail: [] },
      { name: 'Body 3', x: 0, y: 1.5e11, vx: 15000, vy: 0, mass: 1e30, color: '#10B981', trail: [] },
    ],
  },
};
...
export default function AstrodynamicsSandbox() {
  ...
  const [bodies, setBodies] = useState<BodyConfig[]>(PRESETS['sun-earth-moon'].bodies.map(b => ({ ...b, trail: [] })));
  ...
  useEffect(() => {
    ...
    if (initialEnergy !== null) {
      const dE = initialEnergy !== 0 ? Math.abs((currentEnergy - initialEnergy) / initialEnergy) : 0;
  ...
  const energyDrift = initialEnergy && initialEnergy !== 0
      ...
      <div className="max-w-[120rem] mx-auto px-4 md:px-[4%] py-6">
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-white/50 font-mono">4th-order Runge-Kutta gravitational N-body integrator</p>
          </div>
        </div>
          ...
          <div className="lg:col-span-3">
            <div className="relative rounded-xl overflow-hidden border border-white/10"
              ...
              <canvas ref={canvasRef} className="w-full" style={{ height: '600px' }}
                onWheel={(e) => { e.preventDefault(); setZoom((z) => Math.max(0.1, Math.min(20, z * (e.deltaY > 0 ? 0.9 : 1.1)))); }} />
              ...
              <div className={`absolute top-4 right-4 px-3 py-1.5 rounded-lg bg-black/60 backdrop-blur-sm border font-mono text-xs ${
                Math.abs(energyDrift) < 0.001 ? 'border-green-500/30 text-green-400' :
                Math.abs(energyDrift) < 0.1 ? 'border-amber-500/30 text-amber-400' : 'border-red-500/30 text-red-400'
              }`}>
                ...
                {Object.entries(PRESETS).map(([key, p]) => (
                  <button key={key} onClick={() => loadPreset(key)}
                    className={`w-full text-left px-3 py-2 rounded-lg text-sm transition-all ${
                      preset === key ? 'bg-green-500/10 border border-green-500/30 text-green-400' : 'border border-white/5 hover:bg-white/[0.05] text-white/60'
                    }`}>
                    {p.name}
                  </button>
                ))}
            ...
            <div className="rounded-xl bg-white/[0.03] border border-white/10 p-4" style={{ backdropFilter: 'blur(20px)' }}>
              ...
              <div className="mb-3">
                <div className="flex justify-between text-xs mb-1">
                  ...
                  <span className="font-mono text-green-400">{(timeStep / 3600).toFixed(1)} hrs</span>
                </div>
                <input type="range" min={3600} max={604800} step={3600} value={timeStep}
              ...
              <div className="mb-3">
                ...
                <input type="range" min={1} max={20} value={stepsPerFrame}
              ...
              <div>
                ...
                <input type="range" min={50} max={1000} step={50} value={trailLength}
            ...
            {userMode === 'student' && (
              <div className="p-4 rounded-xl bg-green-500/5 border border-green-500/10">
                <p className="text-xs text-green-400/80">
                  💡 <strong>RK4 Integration</strong> uses 4 force evaluations per step for high accuracy. Watch the ΔE/E₀ indicator — values near 0% mean energy is conserved (physically correct simulation).
                </p>
              </div>
            )}
          </div>
