...
const DEEP_SKY_CATALOG: DeepSkyObject[] = [
  {
    ...
    color: "#00F0FF",
    description:
      "Nearest major galaxy to the Milky Way. Contains ~1 trillion stars. Approaching at 110 km/s — will merge with Milky Way in ~4.5 Gyr.",
  },
  {
    id: "m42",
    name: "Orion Nebula",
    ...
    color: "#FF007A",
    description:
      "Massive stellar nursery with ~700 stars in various stages of formation. Contains the Trapezium Cluster of hot, young O-type and B-type stars.",
  },
  {
    id: "m45",
    name: "Pleiades",
    ...
    color: "#F59E0B",
    description:
      "Young open cluster (~100 Myr). Contains >1,000 confirmed members. Blue reflection nebulosity from interstellar dust.",
  },
  {
    id: "m1",
    name: "Crab Nebula",
    ...
    color: "#A78BFA",
    description:
      "Remnant of SN 1054 supernova observed by Chinese astronomers. Contains a 33 ms pulsar (PSR B0531+21) spinning at 30.2 Hz.",
  },
  {
    id: "m87",
    name: "Virgo A",
    ...
    color: "#10B981",
    description:
      "Supergiant elliptical galaxy with ~12,000 globular clusters. Central SMBH (M87*) imaged by EHT in 2019 — mass 6.5×10⁹ M☉.",
  },
  {
    id: "m13",
    ...
    color: "#06B6D4",
    description:
      "Contains ~300,000 stars in a sphere ~145 ly across. One of the brightest northern globular clusters. Age ~11.65 Gyr.",
  },
  {
    id: "ngc7000",
    ...
    color: "#EF4444",
    description:
      "Large emission nebula resembling the North American continent. Ionized by hot star HD 199579 (O-type). Part of the Cygnus Wall complex.",
  },
  {
    id: "m57",
    name: "Ring Nebula",
    ...
    color: "#EC4899",
    description:
      "Archetypal planetary nebula. Central white dwarf (T ~120,000 K) illuminates ejected stellar envelope. True shape is a barrel, not a ring.",
  },
];
...
export default function DeepSpace() {
  ...
  // Snap to object
  const snapToObject = useCallback(
    (obj: DeepSkyObject) => {
  ...
  const filteredObjects =
    searchQuery.length > 0
      ? DEEP_SKY_CATALOG.filter(
          (o) =>
            o.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
            o.catalog.toLowerCase().includes(searchQuery.toLowerCase()),
        )
      : DEEP_SKY_CATALOG;
      ...
      <div className="max-w-[120rem] mx-auto px-4 md:px-[4%] py-6">
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-white/50 font-mono">
              Interactive sky map · 8 deep-sky targets · RA/Dec coordinate grid
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <div className="lg:col-span-3">
            ...
            >
              ...
              <div className="absolute bottom-4 left-4 bg-black/60 backdrop-blur-sm rounded-lg px-3 py-2 border border-white/10 font-mono text-xs text-white/50">
                Center: {viewCenter.ra.toFixed(2)}h / {viewCenter.dec > 0 ? "+" : ""}
                {viewCenter.dec.toFixed(1)}° · Zoom: {zoom.toFixed(1)}×
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            {/* Object list */}
            <div
              className="rounded-xl bg-white/[0.03] border border-white/10 p-3 max-h-[300px] overflow-y-auto space-y-1.5"
              style={{ backdropFilter: "blur(20px)" }}
            >
              {filteredObjects.map((obj) => (
                <button
                  key={obj.id}
                  onClick={() => snapToObject(obj)}
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-mono text-xs text-white/70">
                      {obj.catalog.split("/")[0].trim()}
                    </span>
                  </div>
                  <span className="text-sm font-medium">{obj.name}</span>
                  <div className="text-xs text-white/30 mt-0.5">
                    {obj.type} · mag {obj.magnitude}
                  </div>
                </button>
              ))}
            </div>
            ...
            {selectedObject && (
              ...
              >
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h3 className="font-semibold text-sm" style={{ color: selectedObject.color }}>
                    {selectedObject.name}
                  </h3>
                </div>
                <div className="space-y-1.5 text-xs font-mono">
                  {[
                    ...
                    {
                      l: "RA",
                      v: `${selectedObject.ra.toFixed(3)}h (${(selectedObject.ra * 15).toFixed(2)}°)`,
                    },
                    {
                      l: "Dec",
                      v: `${selectedObject.dec > 0 ? "+" : ""}${selectedObject.dec.toFixed(3)}°`,
                    },
                ...
                {userMode === "student" && (
                  <div className="mt-3 p-2.5 rounded-lg bg-pink-500/5 border border-pink-500/10">
                    <p className="text-xs text-pink-400/70">
                      💡 Right Ascension (RA) measures the angular distance eastward along the
                      celestial equator. 1 hour of RA = 15°. Declination (Dec) is the angular
