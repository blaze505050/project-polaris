...
const VEHICLE_PRESETS = [
  ...
  { name: 'Apollo Command Module', v0: 11000, gamma0: -6.5, ballisticCoeff: 380, ld: 0.35, radius: 2.0 },
  { name: 'Space Shuttle Orbiter', v0: 7850, gamma0: -1.2, ballisticCoeff: 220, ld: 1.5, radius: 1.2 },
  { name: 'Hypersonic Glide Vehicle (HGV)', v0: 6500, gamma0: -2.0, ballisticCoeff: 180, ld: 3.0, radius: 0.5 },
];
...
const REENTRY_METADATA: SimulationMetadata = {
  ...
  unitsTable: [
    { symbol: 'q', name: 'Stagnation Heat Flux', unit: 'W/cm²', description: 'Convective heat transfer rate at nose stagnation point' },
    { symbol: 'B', name: 'Ballistic Coefficient', unit: 'kg/m²', description: 'Vehicle mass divided by drag area factor' },
    { symbol: 'γ', name: 'Flight Path Angle', unit: 'degrees', description: 'Angle of velocity vector relative to local horizon' },
    { symbol: 'N_g', name: 'Deceleration G-Load', unit: 'G', description: 'Total non-gravitational acceleration divided by 9.80665 m/s²' },
  ],
  references: [
...
export default function HypersonicReentryCorridor() {
  ...
  const result: ReentryCorridorResult = useMemo(() => {
    return computeReentryTrajectory(entryVelocity, entryAngle, ballisticCoeff, liftToDrag, noseRadius);
  ...
  const solverState = (result.thermalFailure || result.structuralFailure || result.skippedOut) ? 'WARNING' : 'CONVERGED';
      ...
      <div className="max-w-[120rem] mx-auto px-4 md:px-[4%] py-6 space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-white/50 font-mono">Sutton-Graves stagnation heat flux · Flight path angle bounds · Deceleration G-loads</p>
            </div>
          </div>
        ...
        <div className="flex gap-2 overflow-x-auto pb-1">
          {VEHICLE_PRESETS.map((v, idx) => (
            <button key={v.name} onClick={() => applyPreset(idx)}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-4">
            <div className="rounded-xl bg-white/[0.03] border border-white/10 p-4" style={{ backdropFilter: 'blur(20px)' }}>
              ...
              {[
                { label: 'Entry Velocity V₀ [m/s]', value: entryVelocity, set: setEntryVelocity, min: 5000, max: 12000, step: 100 },
                { label: 'Flight Path Angle γ₀ [°]', value: entryAngle, set: setEntryAngle, min: -12, max: -0.5, step: 0.1 },
                { label: 'Ballistic Coeff B [kg/m²]', value: ballisticCoeff, set: setBallisticCoeff, min: 50, max: 800, step: 10 },
                { label: 'Lift-to-Drag Ratio [L/D]', value: liftToDrag, set: setLiftToDrag, min: 0, max: 4.0, step: 0.1 },
                { label: 'Nose Radius R_n [m]', value: noseRadius, set: setNoseRadius, min: 0.2, max: 5.0, step: 0.1 },
              ].map((p) => (
            ...
            <div className="rounded-xl bg-white/[0.03] border border-white/10 p-4" style={{ backdropFilter: 'blur(20px)' }}>
              ...
              <div className="space-y-2 font-mono text-xs">
                <div className="flex justify-between border-b border-white/5 py-1.5">
                  ...
                  <span className={`font-bold ${result.peakHeatFlux > 400 ? 'text-red-400' : 'text-amber-400'}`}>
                    {result.peakHeatFlux.toFixed(1)} W/cm²
                  </span>
                </div>
                <div className="flex justify-between border-b border-white/5 py-1.5">
                  <span className="text-white/40">Peak Deceleration [N_g]</span>
                  <span className={`font-bold ${result.peakGLoad > 12 ? 'text-red-400' : 'text-cyan-400'}`}>
                    {result.peakGLoad.toFixed(1)} Gs
                  </span>
                </div>
                <div className="flex justify-between border-b border-white/5 py-1.5">
                  <span className="text-white/40">Max Dynamic Pressure [q_dyn]</span>
                  <span className="text-white">{(result.maxDynamicPressure / 1000).toFixed(1)} kPa</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <ExperimentHistoryLogger
          ...
          currentOutputs={{ peakHeatFlux_W_cm2: result.peakHeatFlux, peakGLoad_G: result.peakGLoad, maxDynamicPressure_kPa: result.maxDynamicPressure / 1000 }}
        />
