...
export default function Coordinates() {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      ctx.fillText(
        directions[i],
        cx + (maxR + 15) * Math.cos(angle),
        cy + (maxR + 15) * Math.sin(angle) + 4,
      );
    }
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-white/50 font-mono">
              RA/Dec → Alt/Az · Julian Date · GMST · Local Sidereal Time
            </p>
          </div>
        </div>
            ...
            <div className="mt-3 text-center">
              <span
                className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-mono ${
                  result.isAboveHorizon
                    ? "bg-green-500/10 text-green-400 border border-green-500/20"
                    : "bg-red-500/10 text-red-400 border border-red-500/20"
                }`}
              >
                <div
                  className={`w-2 h-2 rounded-full ${result.isAboveHorizon ? "bg-green-400 animate-pulse" : "bg-red-400"}`}
                />
              ...
              <div className="space-y-1.5">
                {[
                  ...
                  >
                    ...
                    <span
                      className={`font-mono text-sm ${row.highlight ? "text-blue-400 font-bold" : "text-white/70"}`}
                    >
                      {row.v}
                    </span>
                  </div>
            ...
            {userMode === "student" && (
              <div className="p-4 rounded-xl bg-blue-500/5 border border-blue-500/10">
                <p className="text-xs text-blue-400/80">
                  💡 <strong>RA/Dec → Alt/Az:</strong> Right Ascension and Declination are fixed sky
                  coordinates. Altitude and Azimuth depend on WHERE and WHEN you observe. The Hour
                  Angle tells you how far the object is from the meridian. Objects with negative
                  altitude are below the horizon.
                </p>
              </div>
            )}
          </div>
