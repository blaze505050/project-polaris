...
export default function OrbitalMechanics() {
  ...
  useEffect(() => {
    ...
    ctx.moveTo(ax, ay);
    ctx.lineTo(
      ax - arrowLen * Math.cos(vAngle - arrowAngle),
      ay + arrowLen * Math.sin(vAngle - arrowAngle),
    );
    ctx.moveTo(ax, ay);
    ctx.lineTo(
      ax - arrowLen * Math.cos(vAngle + arrowAngle),
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-white/50 font-mono">
              Complete orbital elements · Vis-viva velocity · Dynamic 2D ellipse
            </p>
          </div>
        </div>
          ...
          <div className="space-y-4">
              ...
              {[
                {
                  ...
                  value: semiMajorAxis,
                  set: setSemiMajorAxis,
                  min: CENTRAL_BODIES[centralBody].radius / 1000 + 100,
                  max: centralBody === "Sun" ? 1e9 : 100000,
                  step: 100,
                },
                {
                  label: "Eccentricity",
            ...
            >
              <div className="flex items-center justify-between">
                ...
                >
                  <div
                    className={`w-4 h-4 rounded-full bg-white transition-transform ${isAnimating ? "translate-x-5" : "translate-x-0.5"}`}
                  />
                </button>
              </div>
            </div>
            ...
            {userMode === "student" && (
              <div className="p-4 rounded-xl bg-emerald-500/5 border border-emerald-500/10">
                <p className="text-xs text-emerald-400/80">
                  💡 <strong>Keplerian Orbits:</strong> The semi-major axis determines the orbit
                  size and period. Eccentricity controls the shape (0 = circle, close to 1 = very
                  elongated). The yellow arrow shows the velocity vector — it's fastest at periapsis
                  (closest approach) and slowest at apoapsis.
                </p>
              </div>
            )}
          </div>
              ...
              >
                ...
                <div className="space-y-1.5">
                  {[
                    {
                      l: "Perigee Altitude",
                      v: `${(solution.periapsisAltitude / 1000).toFixed(1)} km`,
                      c: "#10B981",
                    },
                    {
                      l: "Apogee Altitude",
                      v: `${(solution.apoapsisAltitude / 1000).toFixed(1)} km`,
                      c: "#EF4444",
                    },
                    { l: "Orbital Period", v: formatTime(solution.orbitalPeriod), c: "#00F0FF" },
                    {
                      l: "Periapsis Velocity",
                      v: `${(solution.periapsisVelocity / 1000).toFixed(3)} km/s`,
                      c: "#10B981",
                    },
                    {
                      l: "Apoapsis Velocity",
                      v: `${(solution.apoapsisVelocity / 1000).toFixed(3)} km/s`,
                      c: "#EF4444",
                    },
                    {
                      l: "Specific Energy",
                      v: `${solution.specificEnergy.toExponential(4)} J/kg`,
                      c: "#F59E0B",
                    },
                    {
                      l: "Angular Momentum",
                      v: `${solution.angularMomentum.toExponential(4)} m²/s`,
                      c: "#A78BFA",
                    },
                    {
                      l: "Mean Motion",
                      v: `${((solution.meanMotion * 180) / Math.PI).toFixed(6)} °/s`,
                      c: "#06B6D4",
                    },
                  ].map((row) => (
                    <div
                ...
                {solution.periapsisAltitude < 0 && (
                  <div className="mt-3 p-2.5 rounded-lg bg-red-500/10 border border-red-500/20">
                    <p className="text-xs text-red-400">
                      ⚠️ Periapsis is below the surface! This orbit would impact {centralBody}.
                    </p>
                  </div>
                )}
              </motion.div>
