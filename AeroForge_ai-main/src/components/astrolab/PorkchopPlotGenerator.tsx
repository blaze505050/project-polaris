...
import { generatePorkchopGrid, type PorkchopPoint, type PorkchopGridResult } from '@/services/physicsEngine';
...
const PRESETS = [
  { name: 'Earth → Mars (2026 Window)', depart: 'Earth' as const, arrive: 'Mars' as const, launchStart: '2026-10-01', launchSpan: 90, arriveStart: '2027-05-01', arriveSpan: 120 },
  { name: 'Earth → Mars (2028 Window)', depart: 'Earth' as const, arrive: 'Mars' as const, launchStart: '2028-11-01', launchSpan: 90, arriveStart: '2029-06-01', arriveSpan: 120 },
  { name: 'Earth → Venus (2026 Window)', depart: 'Earth' as const, arrive: 'Venus' as const, launchStart: '2026-03-01', launchSpan: 60, arriveStart: '2026-07-01', arriveSpan: 90 },
  { name: 'Earth → Jupiter (2026 Window)', depart: 'Earth' as const, arrive: 'Jupiter' as const, launchStart: '2026-04-01', launchSpan: 90, arriveStart: '2027-10-01', arriveSpan: 180 },
];
...
const PORKCHOP_METADATA: SimulationMetadata = {
  ...
  unitsTable: [
    { symbol: 'C3', name: 'Characteristic Energy', unit: 'km²/s²', description: 'Square of departure hyperbolic excess velocity' },
    { symbol: 'ΔV', name: 'Total Velocity Change', unit: 'km/s', description: 'Combined departure and arrival impulse cost' },
    { symbol: 'v_inf', name: 'Hyperbolic Excess Velocity', unit: 'km/s', description: 'Spacecraft speed relative to planet at infinity' },
    { symbol: 'TOF', name: 'Time of Flight', unit: 'days', description: 'Total transfer duration in solar days' },
  ],
  references: [
...
export default function PorkchopPlotGenerator() {
  ...
  const [arrivePlanet, setArrivePlanet] = useState<'Mars' | 'Jupiter' | 'Venus'>(selectedPreset.arrive);
  ...
  useEffect(() => {
    ...
    if (gridResult.minDeltaVPoint) {
      ...
      const colIdx = Math.floor(((minPt.launchDate - launchStartMs) / (launchDaysSpan * 86400 * 1000)) * cols);
      const rowIdx = Math.floor(((minPt.arrivalDate - arrivalStartMs) / (arrivalDaysSpan * 86400 * 1000)) * rows);
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    const rowIdx = Math.min(rows - 1, Math.max(0, Math.floor(((margin.top + plotH - my) / plotH) * rows)));
      ...
      <div className="max-w-[120rem] mx-auto px-4 md:px-[4%] py-6 space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-white/50 font-mono">Lambert p-iteration solver · Launch window optimization · C₃ characteristic energy</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2">
            <div className="relative rounded-xl overflow-hidden border border-white/10 bg-[#080d1a]"
            ...
            <div className="rounded-xl bg-white/[0.03] border border-white/10 p-4" style={{ backdropFilter: 'blur(20px)' }}>
              ...
              <div className="grid grid-cols-2 gap-3 mb-3">
                <div>
                  ...
                  <select value={departPlanet} onChange={(e) => setDepartPlanet(e.target.value as any)}
                ...
                <div>
                  ...
                  <select value={arrivePlanet} onChange={(e) => setArrivePlanet(e.target.value as any)}
                    className="w-full bg-black/40 border border-white/10 rounded px-2.5 py-1.5 text-xs font-mono text-white focus:outline-none">
              ...
              <div className="space-y-2">
                <div>
                  <label className="text-xs text-white/40 block mb-1">Launch Date Start [UTC]</label>
                  <input type="date" value={launchStartStr} onChange={(e) => setLaunchStartStr(e.target.value)}
                    className="w-full bg-black/40 border border-white/10 rounded px-2.5 py-1.5 text-xs font-mono text-white focus:outline-none" />
                </div>
                <div>
                  <label className="text-xs text-white/40 block mb-1">Arrival Date Start [UTC]</label>
                  <input type="date" value={arrivalStartStr} onChange={(e) => setArrivalStartStr(e.target.value)}
                    className="w-full bg-black/40 border border-white/10 rounded px-2.5 py-1.5 text-xs font-mono text-white focus:outline-none" />
                </div>
              </div>
            </div>
              ...
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                ...
                <div className="space-y-2 font-mono text-xs">
                  <div className="flex justify-between border-b border-white/5 py-1">
                    ...
                    <span className="text-cyan-400">{new Date(minPoint.launchDate).toISOString().slice(0, 10)}</span>
                  </div>
                  <div className="flex justify-between border-b border-white/5 py-1">
                    <span className="text-white/40">Arrival Date</span>
                    <span className="text-emerald-400">{new Date(minPoint.arrivalDate).toISOString().slice(0, 10)}</span>
                  </div>
                  <div className="flex justify-between border-b border-white/5 py-1">
                  ...
                  <div className="flex justify-between border-b border-white/5 py-1">
                    ...
                    <span className="text-amber-400 font-bold">{minPoint.c3.toFixed(2)} km²/s²</span>
                  </div>
                  <div className="flex justify-between border-b border-white/5 py-1">
                  ...
                  <div className="flex justify-between border-b border-white/5 py-1">
                    ...
                    <span className="text-cyan-400 font-bold text-sm">{minPoint.totalDeltaV.toFixed(2)} km/s</span>
                  </div>
                </div>
              </motion.div>
            )}
        ...
        <ExperimentHistoryLogger
          ...
          currentInputs={{ departPlanet, arrivePlanet, launchStartStr, arrivalStartStr, launchDaysSpan, arrivalDaysSpan }}
          currentOutputs={minPoint ? { totalDeltaV_km_s: minPoint.totalDeltaV, c3_km2_s2: minPoint.c3, tof_days: minPoint.tofDays } : {}}
        />
