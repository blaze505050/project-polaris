...
export default function AstroLabSimulationsPage() {
  const [activeTab, setActiveTab] = useState<"orbital" | "gravity" | "transit" | "stellar">(
    "orbital",
  );
  ...
  const [orbitalState, setOrbitalState] = useState(
    computeOrbitalState(orbitalMass, orbitalRadius, orbitalEccentricity),
  );
  ...
  const [transitCurve, setTransitCurve] = useState(
    generateTransitLightCurve(planetRadius, starRadius, orbitalPeriod),
  );
  ...
  const handleSaveExperiment = () => {
    ...
    if (activeTab === "orbital") {
      ...
      results = {
        transitDepth: transitCurve.transitDepth,
        transitDuration: transitCurve.transitDuration,
      };
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    <Slider
                      value={[orbitalMass]}
                      onValueChange={(val) =>
                        handleOrbitalUpdate(val[0], orbitalRadius, orbitalEccentricity)
                      }
                  ...
                  <div>
                    ...
                    <Slider
                      value={[orbitalRadius]}
                      onValueChange={(val) =>
                        handleOrbitalUpdate(orbitalMass, val[0], orbitalEccentricity)
                      }
                      min={6.371e6}
                      max={1.496e11}
                      step={1e6}
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div>
                        ...
                        <p className="text-cyan-400 font-mono">
                          {(orbitalState.velocity / 1000).toFixed(2)} km/s
                        </p>
                      </div>
                      <div>
                        <p className="text-slate-400">Period</p>
                        <p className="text-cyan-400 font-mono">
                          {(orbitalState.period / 86400).toFixed(2)} days
                        </p>
                      </div>
                      <div>
                        <p className="text-slate-400">Escape Velocity</p>
                        <p className="text-cyan-400 font-mono">
                          {(orbitalState.escapeVelocity / 1000).toFixed(2)} km/s
                        </p>
                      </div>
                      <div>
                        <p className="text-slate-400">Specific Energy</p>
                        <p className="text-cyan-400 font-mono">
                          {orbitalState.specificOrbitalEnergy.toFixed(0)} J/kg
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-slate-700 p-4 rounded-lg">
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div>
                        ...
                        <p className="text-cyan-400 font-mono">
                          {gravityResult.force.toExponential(2)} N
                        </p>
                      </div>
                      <div>
                        <p className="text-slate-400">Acceleration</p>
                        <p className="text-cyan-400 font-mono">
                          {gravityResult.acceleration.toFixed(4)} m/s²
                        </p>
                      </div>
                    </div>
                    <p className="text-xs text-slate-300 mt-2">
                      {gravityResult.distanceValidation}
                    </p>
                  </div>
                </div>
              )}
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    <Slider
                      value={[planetRadius]}
                      onValueChange={(val) =>
                        handleTransitUpdate(val[0], starRadius, orbitalPeriod)
                      }
                      min={1e6}
                      max={starRadius}
                      step={1e5}
                  ...
                  <div>
                    ...
                    <Slider
                      value={[starRadius]}
                      onValueChange={(val) =>
                        handleTransitUpdate(planetRadius, val[0], orbitalPeriod)
                      }
                      min={planetRadius}
                  ...
                  <div className="bg-slate-700 p-4 rounded-lg">
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div>
                        ...
                        <p className="text-cyan-400 font-mono">
                          {transitCurve.transitDepth.toFixed(3)}%
                        </p>
                      </div>
                      <div>
                        <p className="text-slate-400">Transit Duration</p>
                        <p className="text-cyan-400 font-mono">
                          {transitCurve.transitDuration.toFixed(2)} hours
                        </p>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div>
                        ...
                        <p className="text-cyan-400 font-mono text-lg">
                          {stellarProps.spectralClass}
                        </p>
                      </div>
                      <div>
                        <p className="text-slate-400">Surface Temperature</p>
                        <p className="text-cyan-400 font-mono">
                          {stellarProps.surfaceTemperature.toLocaleString()} K
                        </p>
                      </div>
                      <div>
                        <p className="text-slate-400">Luminosity</p>
                        <p className="text-cyan-400 font-mono">
                          {stellarProps.luminosity.toFixed(2)} L☉
                        </p>
                      </div>
                      <div>
                        <p className="text-slate-400">Main Sequence Lifetime</p>
                        <p className="text-cyan-400 font-mono">
                          {(stellarProps.mainSequenceLifetime / 1e9).toFixed(2)} Gy
                        </p>
                      </div>
                    </div>
                  </div>
