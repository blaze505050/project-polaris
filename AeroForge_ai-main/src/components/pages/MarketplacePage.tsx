  ...
  const items: MarketplaceItem[] = [
    {
      ...
      rating: 4.9,
      description:
        "High-speed supercritical wing section optimized for Mach 0.78 transport aircraft. Includes 2D structured mesh & polar data.",
    ...
    },
    {
      ...
      rating: 5.0,
      description:
        "Full CR3BP Earth-Moon L2 halo orbit numerical propagator with stationkeeping delta-V budgets.",
    ...
    },
    {
      ...
      rating: 4.8,
      description:
        "Euler-Bernoulli anisotropic composite beam deflection and ply failure criterion solver package.",
    ...
    },
    {
      ...
      rating: 4.9,
      description:
        "Fay-Riddell stagnation heat flux solver with high-temperature equilibrium air properties up to Mach 25.",
      ...
      <main className="flex-1 max-w-[120rem] w-full mx-auto px-[6%] py-12">
        ...
        <section className="mb-12 text-center max-w-4xl mx-auto space-y-4">
          ...
          <p className="text-white/70 text-base font-sans leading-relaxed">
            Discover verified airfoil sections, orbital notebooks, and composite structural
            templates built by computational engineers worldwide. Publish your research and earn 85%
            creator royalties.
          </p>
          ...
          {filteredItems.map((item) => (
            ...
            >
              <div className="space-y-3">
                ...
                <h3 className="font-heading text-sm font-bold text-white line-clamp-2">
                  {item.title}
                </h3>
                <p className="text-[11px] text-white/40 font-mono">By {item.creator}</p>
                <p className="text-xs text-white/60 line-clamp-3 font-sans leading-relaxed">
                  {item.description}
                </p>
                ...
                <div className="flex flex-wrap gap-1 pt-1">
                  {item.tags.map((t) => (
                    <span
                      key={t}
                      className="text-[9px] font-mono px-2 py-0.5 bg-cyan-500/10 text-cyan-400 rounded border border-cyan-500/20"
                    >
                      #{t}
                    </span>
                  ))}
        ...
        <section className="bg-[#0A1020] border border-white/10 rounded-2xl p-6 md:p-10 space-y-8 shadow-xl">
          <div className="text-center max-w-2xl mx-auto space-y-2">
            ...
            <p className="text-xs text-white/60 font-sans">
              Choose the right access tier for individual researchers, computational labs, and
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 font-mono">
            ...
            <div className="bg-[#050A16] border border-white/10 rounded-xl p-6 space-y-4">
              ...
              <p className="text-xs text-white/60 font-sans">
                100% free client-side access to all 54 reduced-order solvers.
              </p>
              <ul className="space-y-2 text-xs text-white/70 font-sans">
                <li className="flex items-center gap-2">
                  <CheckCircle2 className="w-4 h-4 text-emerald-400" /> All 54 Local Solvers
                </li>
                <li className="flex items-center gap-2">
                  <CheckCircle2 className="w-4 h-4 text-emerald-400" /> .aeroforge JSON Export
                </li>
                <li className="flex items-center gap-2">
                  <CheckCircle2 className="w-4 h-4 text-emerald-400" /> Public Artifact Links
                </li>
              </ul>
            </div>
            ...
            <div className="bg-[#050A16] border border-cyan-500/50 rounded-xl p-6 space-y-4 relative shadow-lg shadow-cyan-500/10">
              ...
              <div className="text-3xl font-extrabold text-white">
                $29 <span className="text-xs font-normal text-white/40">/ month</span>
              </div>
              <p className="text-xs text-white/60 font-sans">
                FastAPI PyTorch Neural Operator cloud access + Creator Marketplace publishing.
              </p>
              <ul className="space-y-2 text-xs text-white/70 font-sans">
                <li className="flex items-center gap-2">
                  <CheckCircle2 className="w-4 h-4 text-cyan-400" /> 14ms PyTorch FNO Inference
                </li>
                <li className="flex items-center gap-2">
                  <CheckCircle2 className="w-4 h-4 text-cyan-400" /> Publish & Earn on Marketplace
                </li>
                <li className="flex items-center gap-2">
                  <CheckCircle2 className="w-4 h-4 text-cyan-400" /> Cloud Artifact Registry API
                </li>
              </ul>
            </div>
            ...
            <div className="bg-[#050A16] border border-purple-500/40 rounded-xl p-6 space-y-4">
              ...
              <div className="text-3xl font-extrabold text-white">
                $199 <span className="text-xs font-normal text-white/40">/ month</span>
              </div>
              <p className="text-xs text-white/60 font-sans">
                Dedicated RANS OpenFOAM & SU2 HPC cluster gateway adapters.
              </p>
              <ul className="space-y-2 text-xs text-white/70 font-sans">
                <li className="flex items-center gap-2">
                  <CheckCircle2 className="w-4 h-4 text-purple-400" /> Slurm / AWS HPC Gateway
                </li>
                <li className="flex items-center gap-2">
                  <CheckCircle2 className="w-4 h-4 text-purple-400" /> Custom Solver Adapters
                </li>
                <li className="flex items-center gap-2">
                  <CheckCircle2 className="w-4 h-4 text-purple-400" /> AS9100 / ISO Data Thread
                  Audit
                </li>
              </ul>
            </div>
