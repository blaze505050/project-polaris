...
export default function AstroLabExoplanetHabitabilityPage() {
  ...
  const habitability = useMemo(() => {
    ...
    result.inHabitableZone =
      orbitalDistance >= recentVenusLimit && orbitalDistance <= earlyMarsLimit;

    // 2. Equilibrium Temperature (Stefan-Boltzmann)
    const albedo =
      atmosphericComposition === "venus-like"
        ? 0.7
        : atmosphericComposition === "mars-like"
          ? 0.25
          : 0.3;
    const solarFlux =
      (starLuminosity * AstronomicalConstants.SOLAR_CONSTANT) / (orbitalDistance * orbitalDistance);
    ...
    const escapeVelocity = Math.sqrt(
      (2 * AstronomicalConstants.GRAVITATIONAL_CONSTANT * planetMassKg) / planetRadiusM,
    );

    // Thermal velocity at exobase (simplified)
    const exobaseTemp = result.equilibriumTemp + 100; // Rough estimate
    const thermalVelocity = Math.sqrt(
      (3 * AstronomicalConstants.BOLTZMANN_CONSTANT * exobaseTemp) / 2e-26,
    ); // H atom mass
          ...
          >
            ...
            <Card className="bg-gradient-to-br from-slate-800/50 to-slate-900/50 border-slate-700 p-8">
              <div className="flex items-start justify-between mb-6">
                ...
                <div
                  className={`text-3xl ${habitability.inHabitableZone ? "text-green-400" : "text-red-400"}`}
                >
            ...
            <div className="grid grid-cols-2 gap-4">
              <Card className="bg-slate-800/50 border-slate-700 p-6">
                ...
                <div className="text-2xl font-bold text-white">
                  {habitability.equilibriumTemp.toFixed(1)}°C
                </div>
                <div className="text-xs text-slate-400 mt-2">
                  Greenhouse: {habitability.greenhouseEffect.toFixed(1)}°C
                </div>
              </Card>
              ...
              <Card className="bg-slate-800/50 border-slate-700 p-6">
                ...
                <div className="text-2xl font-bold text-white">
                  {(habitability.waterPresence * 100).toFixed(0)}%
                </div>
              ...
              <Card className="bg-slate-800/50 border-slate-700 p-6">
                ...
                <div className="text-2xl font-bold text-white">
                  {(habitability.atmosphericRetention * 100).toFixed(0)}%
                </div>
                <div className="w-full bg-slate-700 rounded-full h-2 mt-2 overflow-hidden">
                  <div
              ...
              <Card className="bg-slate-800/50 border-slate-700 p-6">
                ...
                <div className="text-2xl font-bold text-white">
                  {(habitability.magneticProtection * 100).toFixed(0)}%
                </div>
                <div className="w-full bg-slate-700 rounded-full h-2 mt-2 overflow-hidden">
                  <div
