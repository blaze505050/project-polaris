...
export default function FlagshipWorkflowPage() {
  usePageMeta(
    "Flagship Workflow — Aerodynamic Research",
    "Guided end-to-end aerodynamic design, analysis, optimization, and documentation workflow.",
  );
  ...
  const machRes = useMemo(
    () => computeMachNumberCalc(velocity, atmosphere.speedOfSound),
    [velocity, atmosphere],
  );
  const reynoldsRes = useMemo(
    () => computeReynoldsNumber(velocity, chord, atmosphere.density, atmosphere.dynamicViscosity),
    [velocity, chord, atmosphere],
  );
  const airfoilData = useMemo(() => generateNACA4Digit(naca), [naca]);
  const aeroCoeffs = useMemo(
    () =>
      computeAirfoilCoefficients(
        aoa,
        airfoilData?.alphaZeroLift || 0,
        0.008,
        8,
        0.85,
        machRes?.machNumber || 0.1,
      ),
    [aoa, airfoilData, machRes],
  );
  ...
  const sweepData = useMemo(() => {
    ...
    for (let angle = -4; angle <= 16; angle += 2) {
      const res = computeAirfoilCoefficients(
        angle,
        airfoilData?.alphaZeroLift || 0,
        0.008,
        8,
        ...
        <div className="mb-6 flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <h1 className="text-2xl md:text-3xl font-extrabold text-white">
              Aerodynamic Research Workflow
            </h1>
            <p className="text-xs text-white/50 mt-0.5">
              End-to-end digital engineering thread connecting requirements, geometry, atmospheric
              conditions, solver execution, optimization, and reporting.
            </p>
          </div>
          ...
          >
              ...
              <div className="space-y-6">
                <div>
                  <h3 className="text-lg font-bold text-white mb-1">
                    Step 1: Define Engineering Requirements
                  </h3>
                  <p className="text-xs text-white/50">
                    Establish mission targets for aerodynamic efficiency and speed envelope.
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-4">
                    <div>
                      <label className="text-xs text-white/70 font-medium block mb-1">
                  ...
                  <div className="bg-[#060B18] p-5 rounded-lg border border-white/5 space-y-2">
                    ...
                    <p className="text-xs text-white/60">
                      • Minimum Cruise L/D Target:{" "}
                      <strong className="text-white">{targetLd}</strong>
                    </p>
                    <p className="text-xs text-white/60">
                      • Cruise Speed Envelope:{" "}
                      <strong className="text-white">{velocity} m/s</strong> (
                      {(velocity * 1.94384).toFixed(1)} knots)
                    </p>
                    <p className="text-xs text-white/60">
                      • Traceability ID:{" "}
                      <strong className="font-mono text-cyan-300">REQ-AERO-2026-01</strong>
                    </p>
                  </div>
                </div>
              </div>
            ...
            {currentStep === 2 && (
              <div className="space-y-6">
                <div>
                  <h3 className="text-lg font-bold text-white mb-1">
                    Step 2: Generate Airfoil Geometry
                  </h3>
                  <p className="text-xs text-white/50">
                    Configure NACA 4-Digit geometry parameters.
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  {airfoilData && (
                    <div className="bg-[#060B18] p-4 rounded-lg border border-white/5 space-y-2 text-xs">
                      <h4 className="font-mono font-bold text-cyan-400 uppercase tracking-wider">
                        Geometry Metrics
                      </h4>
                      <p className="text-white/60">
                        Max Camber:{" "}
                        <span className="font-mono text-white font-bold">
                          {(airfoilData.maxCamber * 100).toFixed(1)}%
                        </span>
                      </p>
                      <p className="text-white/60">
                        Max Thickness:{" "}
                        <span className="font-mono text-white font-bold">
                          {(airfoilData.maxThickness * 100).toFixed(1)}%
                        </span>
                      </p>
                      <p className="text-white/60">
                        Zero-Lift Angle (α₀):{" "}
                        <span className="font-mono text-white font-bold">
                          {airfoilData.alphaZeroLift.toFixed(2)}°
                        </span>
                      </p>
                    </div>
                  )}
            ...
            {currentStep === 3 && (
              <div className="space-y-6">
                <div>
                  <h3 className="text-lg font-bold text-white mb-1">
                    Step 3: Atmospheric & Flow Regime
                  </h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="bg-[#060B18] p-4 rounded-lg border border-white/5 space-y-2 text-xs">
                    ...
                    <p className="text-white/60">
                      Air Density:{" "}
                      <span className="font-mono text-white font-bold">
                        {atmosphere.density.toFixed(4)} kg/m³
                      </span>
                    </p>
                    <p className="text-white/60">
                      Mach Number:{" "}
                      <span className="font-mono text-white font-bold">
                        {machRes?.machNumber.toFixed(3)} ({machRes?.regime})
                      </span>
                    </p>
                    <p className="text-white/60">
                      Reynolds Number:{" "}
                      <span className="font-mono text-white font-bold">
                        {reynoldsRes?.reynoldsNumber.toExponential(3)} ({reynoldsRes?.regime})
                      </span>
                    </p>
                  </div>
                </div>
                  ...
                  <div className="bg-[#060B18] p-4 rounded-lg border border-white/5 space-y-2 text-xs">
                    ...
                    <p className="text-white/60">
                      ...
                      <span className="font-mono text-cyan-300 font-bold">
                        {aeroCoeffs.cl.toFixed(4)}
                      </span>
                    </p>
                    <p className="text-white/60">
                      Drag Coefficient (CD):{" "}
                      <span className="font-mono text-white font-bold">
                        {aeroCoeffs.cd.toFixed(4)}
                      </span>
                    </p>
                    <p className="text-white/60">
                      L/D Ratio:{" "}
                      <span className="font-mono text-emerald-400 font-bold">
                        {aeroCoeffs.ldRatio.toFixed(2)}
                      </span>
                    </p>
                  </div>
                </div>
            ...
            {currentStep === 5 && (
              <div className="space-y-6">
                <div>
                  <h3 className="text-lg font-bold text-white mb-1">
                    Step 5: Angle of Attack Parameter Sweep
                  </h3>
                  <p className="text-xs text-white/50">
                    Multi-point polar generation across operating envelope.
                  </p>
                </div>
                  ...
                  <table className="w-full text-xs font-mono border-collapse">
                    ...
                    <tbody>
                      {sweepData.map((pt) => (
                        <tr
                          key={pt.angle}
                          className={`border-b border-white/5 ${pt.angle === aoa ? "bg-cyan-500/10" : ""}`}
                        >
            ...
            {currentStep === 6 && (
              <div className="space-y-6">
                <div>
                  <h3 className="text-lg font-bold text-white mb-1">
                    Step 6: Pareto Optimization Results
                  </h3>
                  <p className="text-xs text-white/50">
                    Optimal AoA identified for maximum aerodynamic efficiency.
                  </p>
                </div>
                ...
                <div className="bg-[#060B18] p-6 rounded-xl border border-cyan-500/30 space-y-3">
                  ...
                  <p className="text-xs text-white/70">
                    ...
                    <strong className="text-cyan-300 font-mono">{optimalPoint.angle}°</strong> with
                    Max L/D ={" "}
                    <strong className="text-emerald-400 font-mono">
                      {optimalPoint.ld.toFixed(2)}
                    </strong>
                    .
                  </p>
                  <p className="text-xs text-white/50">
                    Requirement Target ({targetLd}):{" "}
                    {optimalPoint.ld >= targetLd ? (
                      <span className="text-emerald-400 font-bold">✔ TARGET MET</span>
                    ) : (
                      <span className="text-amber-400 font-bold">⚠ BELOW TARGET</span>
                    )}
                  </p>
                </div>
              </div>
            ...
            {currentStep === 7 && (
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-xs text-white/50">
                    Complete engineering digital thread documentation ready for publication.
                  </p>
                </div>
                ...
                <div className="bg-[#060B18] p-6 rounded-xl border border-white/10 space-y-4">
                  ...
                  <p className="text-xs text-white/60 leading-relaxed font-mono">
                    Airfoil NACA {naca} evaluated at {altitude}m altitude and {velocity} m/s (Mach{" "}
                    {machRes?.machNumber.toFixed(3)}). Max L/D ratio of {optimalPoint.ld.toFixed(2)}{" "}
                    achieved at AoA = {optimalPoint.angle}°.
                  </p>

                  <div className="flex flex-wrap gap-3 pt-2">
                    <button
                      onClick={handleSaveWorkflow}
                      className="px-4 py-2 rounded-lg bg-cyan-500 text-black text-xs font-mono font-bold hover:bg-cyan-400 transition-all"
                    >
                      Save to Experiments
                    </button>
                    <Link to="/projects">
