...
export default function PricingPage() {
  ...
  const plans = [
    ...
    },
    {
      ...
      buttonStyle:
        "border border-pink-500/40 bg-pink-500/10 hover:bg-pink-500/20 text-pink-300 font-bold",
      features: [
        ...
        <main className="flex-1 lg:ml-64 p-4 md:p-8 space-y-10 max-w-7xl mx-auto">
          ...
          <div className="text-center space-y-3">
            ...
            <p className="text-sm text-white/60 font-sans max-w-2xl mx-auto leading-relaxed">
              Start free with local WASM physics solvers, or unlock cloud HPC clusters, OpenFOAM
              remote solvers, and AS9100 compliance logging.
            </p>

            {/* Monthly / Annual Toggle */}
            <div className="pt-4 flex items-center justify-center gap-3 text-xs">
              <span
                className={billingCycle === "monthly" ? "text-white font-bold" : "text-white/40"}
              >
                Monthly Billing
              </span>
              <button
              ...
              <span
                className={billingCycle === "annual" ? "text-cyan-400 font-bold" : "text-white/40"}
              >
                Annual Billing{" "}
                <span className="text-[10px] text-emerald-400 font-bold ml-1">(Save 20%)</span>
              </span>
            </div>
          </div>
              ...
              >
                <div>
                  <div className="flex items-center justify-between">
                    <span
                      className={`text-[10px] px-2.5 py-0.5 rounded border uppercase ${plan.badgeColor}`}
                    >
                      {plan.badge}
                    </span>
                  </div>
          ...
          <div className="bg-[#080E1C] border border-white/10 rounded-xl p-6 space-y-4">
            <div className="flex items-center justify-between border-b border-white/10 pb-4">
              <div>
                ...
                <p className="text-xs text-white/50 font-sans mt-0.5">
                  Specialized aerospace & propulsion solvers available for Pro & Enterprise
                  accounts.
                </p>
              </div>
              <Link
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {premiumTools.map((tool, i) => (
                <div
                  key={i}
                  className="bg-[#050914] border border-white/5 rounded-lg p-3.5 space-y-1.5"
                >
                  ...
                  <p className="text-[11px] text-white/50 font-sans leading-relaxed">{tool.desc}</p>
                </div>
              ))}
            </div>
          </div>
