...
export default function AstroLabStellarEvolutionPage() {
  ...
  const getHRDiagramPosition = () => {
    ...
    const x =
      ((Math.log10(stellarModel.temperature) - Math.log10(tempRange[0])) /
        (Math.log10(tempRange[1]) - Math.log10(tempRange[0]))) *
      100;

    const y =
      100 -
      ((Math.log10(stellarModel.luminosity) - Math.log10(lumRange[0])) /
        (Math.log10(lumRange[1]) - Math.log10(lumRange[0]))) *
        100;
      ...
      <main className="max-w-7xl mx-auto px-4 py-12">
        ...
        >
          ...
          <p className="text-xl text-slate-300">
            Explore the life cycle of stars from birth to death using Hertzsprung-Russell diagram
            principles
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          >
            {/* Current Stage */}
            <Card
              className={`border-2 p-8 ${stageBgColor[stellarModel.stage as keyof typeof stageBgColor]}`}
            >
              <div className="flex items-start justify-between mb-4">
                <div>
                  ...
                  <h2
                    className={`text-4xl font-bold ${stageColor[stellarModel.stage as keyof typeof stageColor]}`}
                  >
                    {stellarModel.stage}
                  </h2>
                </div>
                <Flame
                  size={40}
                  className={stageColor[stellarModel.stage as keyof typeof stageColor]}
                />
              </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <Card className="bg-slate-800/50 border-slate-700 p-6">
                ...
                <div className="text-2xl font-bold text-white">
                  {stellarModel.radius.toFixed(2)} R☉
                </div>
                <div className="text-xs text-slate-400 mt-2">
                  {((stellarModel.radius * AstronomicalConstants.SOLAR_RADIUS) / 1e9).toFixed(2)}{" "}
                  million km
                </div>
              </Card>
              ...
              <Card className="bg-slate-800/50 border-slate-700 p-6">
                ...
                <div className="text-2xl font-bold text-white">
                  {stellarModel.luminosity.toFixed(2)} L☉
                </div>
                <div className="text-xs text-slate-400 mt-2">
                  {(
                    (stellarModel.luminosity * AstronomicalConstants.SOLAR_LUMINOSITY) /
                    1e26
                  ).toFixed(2)}{" "}
                  × 10²⁶ W
                </div>
              ...
              <Card className="bg-slate-800/50 border-slate-700 p-6">
                ...
                <div className="text-2xl font-bold text-white">
                  {stellarModel.temperature.toFixed(0)} K
                </div>
                <div className="text-xs text-slate-400 mt-2">
              ...
              <Card className="bg-slate-800/50 border-slate-700 p-6">
                ...
                <div className="text-2xl font-bold text-white">
                  {stellarModel.mass.toFixed(1)} M☉
                </div>
                <div className="text-xs text-slate-400 mt-2">
                  {((stellarModel.mass * AstronomicalConstants.SOLAR_MASS) / 1e30).toFixed(1)} ×
                  10³⁰ kg
                </div>
              </Card>
            </div>
            ...
            <Card className="bg-slate-800/50 border-slate-700 p-6">
              ...
              <div className="space-y-3 text-sm text-slate-300">
                <div>
                  <span className="text-blue-400 font-semibold">Main Sequence:</span> Hydrogen
                  burning in core. Longest phase (90% of lifetime).
                </div>
                <div>
                  <span className="text-red-400 font-semibold">Red Giant:</span> Hydrogen shell
                  burning. Core contracts, envelope expands.
                </div>
                <div>
                  <span className="text-yellow-400 font-semibold">Horizontal Branch:</span> Helium
                  burning in core. Brief stable phase.
                </div>
                <div>
                  <span className="text-orange-400 font-semibold">Asymptotic Giant:</span> Helium
                  and hydrogen shell burning. Rapid mass loss.
                </div>
                <div>
                  <span className="text-slate-300 font-semibold">White Dwarf:</span> Cooling
                  remnant. No fusion. Lifetime: trillions of years.
                </div>
              </div>
            </Card>
          </motion.div>
