...
export default function AstroLabCelestialCoordinatePage() {
  ...
  const [sourceType, setSourceType] = useState<"equatorial" | "ecliptic" | "galactic">(
    "equatorial",
  );
  ...
  const toGalactic = useMemo(() => {
    ...
    const l =
      (Math.atan2(
        z * Math.cos(gc_dec_rad) -
          (x * Math.sin(gc_ra_rad) - y * Math.cos(gc_ra_rad)) * Math.sin(gc_dec_rad),
    ...
    const b =
      (Math.asin(
        z * Math.sin(gc_dec_rad) +
          (x * Math.sin(gc_ra_rad) - y * Math.cos(gc_ra_rad)) * Math.cos(gc_dec_rad),
      ) *
        180) /
      Math.PI;
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-secondary-foreground text-sm">
                  Ephemeris calculations & coordinate transformations
                </p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-4">
              <div className="bg-[#131924]/60 backdrop-blur-md border border-[#00F0FF33] rounded-lg p-4">
                <h3 className="text-sm font-mono font-bold text-[#00F0FF] mb-4">
                  Input Coordinates
                </h3>
            ...
            <div className="lg:col-span-2 space-y-4">
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="bg-[#0B0E14] p-3 rounded border border-[#00F0FF33]">
                    ...
                    <div className="text-lg font-mono font-bold text-[#00F0FF] mb-2">
                      {ra.toFixed(4)}h
                    </div>
                    <div className="text-xs text-secondary-foreground mb-2">
                      {Math.floor(ra)}h {Math.floor((ra % 1) * 60)}m{" "}
                      {((((ra % 1) * 60) % 1) * 60).toFixed(1)}s
                    </div>
                    <button
                  ...
                  <div className="bg-[#0B0E14] p-3 rounded border border-[#FF007A33]">
                    ...
                    <div className="text-lg font-mono font-bold text-[#FF007A] mb-2">
                      {dec.toFixed(4)}°
                    </div>
                    <div className="text-xs text-secondary-foreground mb-2">
                      {Math.floor(Math.abs(dec))}° {Math.floor((Math.abs(dec) % 1) * 60)}'{" "}
                      {((((Math.abs(dec) % 1) * 60) % 1) * 60).toFixed(1)}"
                    </div>
                    <button
              ...
              >
                <h3 className="text-sm font-mono font-bold text-[#F59E0B] mb-4">
                  Ecliptic Coordinates
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="bg-[#0B0E14] p-3 rounded border border-[#F59E0B33]">
                    ...
                    <div className="text-lg font-mono font-bold text-[#F59E0B]">
                      {toEcliptic.longitude.toFixed(4)}°
                    </div>
                    <button
              ...
              >
                <h3 className="text-sm font-mono font-bold text-[#A78BFA] mb-4">
                  Galactic Coordinates
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="bg-[#0B0E14] p-3 rounded border border-[#A78BFA33]">
                    ...
                    <div className="text-lg font-mono font-bold text-[#A78BFA]">
                      {toGalactic.longitude.toFixed(4)}°
                    </div>
                    <button
              ...
              >
                <h3 className="text-sm font-mono font-bold text-[#10B981] mb-4">
                  Cartesian Coordinates
                </h3>
