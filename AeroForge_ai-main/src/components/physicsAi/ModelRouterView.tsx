...
import { Cpu, ArrowRight, CheckCircle2, AlertTriangle, Layers, Sliders, Sparkles } from 'lucide-react';
...
export default function ModelRouterView() {
  ...
  const [speedRequirement, setSpeedRequirement] = useState<ModelRouterRequest['speedRequirement']>('real-time');
    ...
    <div className="space-y-6">
      ...
      <div className="bg-[#0A1020] border border-cyan-500/20 rounded-xl p-5 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          ...
          <h2 className="text-xl font-extrabold font-mono text-white">Intelligent Neural Operator Dispatcher</h2>
          <p className="text-xs text-white/60 mt-1 max-w-2xl font-sans">
            Automatically routes your engineering intent, geometry discretization, and PDE constraints to the optimal neural operator architecture.
          </p>
        </div>
        ...
        <div className="bg-[#060B18] px-4 py-2 rounded-lg border border-white/10 font-mono text-xs text-right">
          ...
          <span className="text-cyan-400 font-bold text-lg">{recommendations.length} Evaluated</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 bg-[#0A1020] border border-white/10 rounded-xl p-5 space-y-4 shadow-xl font-mono text-xs">
          ...
          <div>
            <label className="text-white/50 text-[11px] block mb-1">PDE / GOVERNING EQUATION TYPE</label>
            <select
              value={pdeType}
              onChange={(e) => setPdeType(e.target.value)}
              className="w-full bg-[#060B18] border border-white/10 rounded px-3 py-2 text-white focus:outline-none focus:border-cyan-500"
            >
              <option value="Incompressible RANS Navier-Stokes">Incompressible RANS Navier-Stokes</option>
              ...
              <option value="Navier-Stokes + Exact Residual Constraints">Navier-Stokes + Exact Residual Constraints</option>
            </select>
          </div>
          ...
          <div>
            ...
            >
              ...
              <option value="Regular Rectangular 2D Spatial Grid">Regular Rectangular 2D Spatial Grid</option>
          ...
          <div>
            <label className="text-white/50 text-[11px] block mb-1">INFERENCE SPEED REQUIREMENT</label>
        ...
        <div className="lg:col-span-7 space-y-4">
          <h3 className="text-sm font-bold font-mono text-white flex items-center justify-between">
            ...
            <span className="text-[10px] text-white/40 font-normal">Sorted by confidence match</span>
          </h3>
            ...
            {recommendations.slice(0, 5).map((rec, idx) => (
              <motion.div
                ...
                className={`bg-[#0A1020] border rounded-xl p-4 shadow-lg transition-all ${
                  idx === 0 ? 'border-cyan-500/50 bg-gradient-to-r from-cyan-950/20 to-[#0A1020]' : 'border-white/10'
                }`}
              >
                <div className="flex items-start justify-between gap-3 mb-2">
                  ...
                  <div className="text-right">
                    ...
                    <span className="text-[10px] font-mono text-white/40 block">Confidence Score</span>
                  </div>
                </div>

                <p className="text-xs text-white/70 font-sans mb-3 pl-8">
                  {rec.reasoning}
                </p>
