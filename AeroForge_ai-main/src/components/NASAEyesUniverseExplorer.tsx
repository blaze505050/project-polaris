...
export default function NASAEyesUniverseExplorer() {
  ...
  const navigateToBody = useCallback(
    (body: CelestialBody) => {
      ...
      const targetPosition = new THREE.Vector3().copy(body.position).add(direction.multiplyScalar(distance));
          ...
          >
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-foreground/70 text-sm leading-relaxed mb-4">{selectedBody.description}</p>

                {/* Telemetry Data */}
                <div className="bg-aerospace-dark/50 rounded p-3 space-y-2 text-xs font-mono text-foreground/60 mb-4">
                  <div className="flex justify-between">
                    <span>Diameter:</span>
                    <span className="text-aerospace-blue">{(selectedBody.size * 2).toLocaleString()} km</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Distance:</span>
                    <span className="text-aerospace-blue">{(selectedBody.distance / 1e6).toLocaleString()} Mkm</span>
                  </div>
                  {selectedBody.temperature && (
                    <div className="flex justify-between">
                      <span>Temperature:</span>
                      <span className="text-aerospace-blue">{selectedBody.temperature.toLocaleString()} K</span>
                    </div>
                  )}
                  {selectedBody.luminosity && (
                    <div className="flex justify-between">
                      <span>Luminosity:</span>
                      <span className="text-aerospace-blue">{selectedBody.luminosity.toFixed(1)} L☉</span>
                    </div>
                  )}
                  {selectedBody.speed && (
                    <div className="flex justify-between">
                      <span>Orbital Speed:</span>
                      <span className="text-aerospace-blue">{selectedBody.speed.toFixed(2)} km/s</span>
                    </div>
                  )}
                </div>
      ...
      >
        ...
        <div className="flex-1 text-center">
          <div className="text-aerospace-blue text-sm font-mono">
            Distance: {(stats.distance / 1e6).toLocaleString('en-US', { maximumFractionDigits: 2 })} Mkm
          </div>
          ...
          >
            ...
            <div className="space-y-2 text-xs">
              ...
              <div className="text-foreground/60 text-xs mt-3 leading-relaxed">
                Click on any celestial body to view details. Use the search to find specific objects.
              </div>
            </div>
          </motion.div>
        )}
