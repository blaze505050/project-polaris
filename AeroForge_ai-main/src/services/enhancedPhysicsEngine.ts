...
export class AtmosphericModel {
  ...
  static getAtmosphericProperties(altitude: number) {
    ...
    const speedOfSound = Math.sqrt(
      PHYSICS_CONSTANTS.GAMMA_AIR * PHYSICS_CONSTANTS.GAS_CONSTANT_AIR * T,
    );
...
export class AerodynamicSolver {
  ...
  static computeDragCoefficient(
    ...
    if (machNumber > 0.8) {
      const laitone =
        (1 + 0.125 * machNumber * machNumber) / Math.sqrt(1 - machNumber * machNumber);
   ...
   */
  static computePressureCoefficient(cp_incompressible: number, machNumber: number): number {
    if (machNumber < 1) {
      const beta = Math.sqrt(1 - machNumber * machNumber);
...
export class ThermalSolver {
  ...
  static computeRadiativeHeatTransfer(
    ...
    return (
      emissivity *
      PHYSICS_CONSTANTS.STEFAN_BOLTZMANN *
      surfaceArea *
...
export class PropulsionSolver {
   ...
   */
  static computeSpecificImpulse(thrust: number, massFlowRate: number): number {
   ...
   */
  static computeTurbineEfficiency(pressureRatio: number, speedParameter: number): number {
    const eta_max = 0.92;
...
export class OrbitalMechanicsSolver {
   ...
   */
  static computeOrbitalVelocity(centralBodyMass: number, orbitalRadius: number): number {
   ...
   */
  static computeOrbitalPeriod(semiMajorAxis: number, centralBodyMass: number): number {
    const n = Math.sqrt(
      (PHYSICS_CONSTANTS.GRAVITATIONAL_CONSTANT * centralBodyMass) / Math.pow(semiMajorAxis, 3),
    );
    return (2 * Math.PI) / n;
  }
   ...
   */
  static computeEscapeVelocity(centralBodyMass: number, radius: number): number {
...
export class NumericalIntegrator {
   ...
   */
  static rk4(state: number[], derivatives: (s: number[]) => number[], dt: number): number[] {
...
export class OptimizationEngine {
  ...
  static pso(
    ...
    const particles = Array(populationSize)
      .fill(null)
      .map(() => bounds.map(([min, max]) => min + Math.random() * (max - min)));

    const velocities = Array(populationSize)
      .fill(null)
      .map(() => bounds.map(([min, max]) => (max - min) * (Math.random() - 0.5)));
