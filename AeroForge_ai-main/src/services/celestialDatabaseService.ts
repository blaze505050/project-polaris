...
const REAL_CELESTIAL_OBJECTS: CelestialObject[] = [
  // Bright Stars
  {
    ...
    size: 2,
    description:
      "Brightest star in the night sky, Alpha Canis Majoris. Binary system with white dwarf companion.",
    spectralType: "A1V",
    luminosity: 26,
    temperature: 9940,
    mass: 2.02,
  ...
  },
  {
    ...
    size: 1.8,
    description:
      "Second brightest star, yellow supergiant in Carina constellation. Extremely luminous.",
    spectralType: "A9Ib",
    luminosity: 13600,
    temperature: 7350,
    mass: 8.6,
  ...
  },
  {
    ...
    size: 1.5,
    description:
      "Bright star in Lyra constellation. One of the brightest stars in the northern hemisphere.",
    spectralType: "A0V",
    luminosity: 40,
    temperature: 9602,
    mass: 2.1,
  ...
  // Nebulae
  {
    ...
    size: 3.5,
    description:
      "Emission nebula, stellar nursery with active star formation. Visible to naked eye.",
    constellation: "Orion",
  ...
  // Galaxies
  {
    ...
    size: 4.5,
    description:
      "Nearest major galaxy, spiral structure similar to Milky Way. Will collide with Milky Way in 4.5 billion years.",
...
class CelestialDatabaseService {
  ...
  getBrightestObjects(limit: number = 50): CelestialObject[] {
    return [...this.objects].sort((a, b) => a.magnitude - b.magnitude).slice(0, limit);
  }
  ...
  getStatistics() {
    return {
      ...
      averageDistance:
        this.objects.reduce((sum, obj) => sum + obj.distance, 0) / this.objects.length,
    };
  }
}
