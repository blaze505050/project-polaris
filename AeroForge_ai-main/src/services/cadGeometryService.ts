...
export class CADGeometryService {
  ...
  private static validateGeometry(
    ...
    if (metrics.meshQuality.aspectRatio > 100) {
      warnings.push(
        `High aspect ratio (${metrics.meshQuality.aspectRatio.toFixed(1)}) - mesh may have quality issues`,
      );
    }
  ...
  private static checkManifoldGeometry(geometry: THREE.BufferGeometry): boolean {
    ...
    for (let i = 0; i < positions.length; i += 9) {
      const edges = [`${i},${i + 3}`, `${i + 3},${i + 6}`, `${i + 6},${i}`];
   ...
   */
  static exportGeometryToSTL(
    geometry: THREE.BufferGeometry,
    filename: string = "aeroforge-model.stl",
  ): void {
