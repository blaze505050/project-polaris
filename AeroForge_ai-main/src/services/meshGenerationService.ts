...
export class MeshGenerationService {
  ...
  static generateAdaptiveMesh(
    ...
    const boundaryLayerMesh = this.generateBoundaryLayerMesh(surfaceNodes, surfaceElements, config);
   ...
   */
  private static extractSurfaceElements(geometry: THREE.BufferGeometry): Array<number[]> {
  ...
  private static generateBoundaryLayerMesh(
    ...
    const numLayers = Math.ceil(
      Math.log(config.maxSize / config.minSize) / Math.log(config.growthRate),
    );
    ...
    for (let layer = 0; layer < numLayers; layer++) {
      for (const element of surfaceElements) {
        ...
        // Bottom triangle
        elements.push([layerNodeIds[layer][n1], layerNodeIds[layer][n2], layerNodeIds[layer][n3]]);
  ...
  private static generateVolumeMesh(
    ...
    const allNodes = [
      ...surfaceNodes.map((n) => ({ x: n.x, y: n.y, z: n.z })),
      ...boundaryLayerMesh.nodes,
    ];
  ...
  private static identifyBoundaries(
    ...
    boundaries.set(
      "inlet",
      surfaceElements.filter((_, i) => i % 3 === 0).map((e) => e[0]),
    );
    boundaries.set(
      "outlet",
      surfaceElements.filter((_, i) => i % 3 === 1).map((e) => e[0]),
    );
    boundaries.set(
      "wall",
      surfaceElements.filter((_, i) => i % 3 === 2).map((e) => e[0]),
    );
