...
export const VIRTUAL_LAB_MODULES: VirtualLabModule[] = [
  // ===== MODULE 1.1: AERODYNAMICS LABS =====
  {
    ...
    description:
      "Complete virtual wind tunnel environment with subsonic to hypersonic capabilities",
    subsystems: [
      {
...
export const getModuleStatistics = () => {
  ...
  const totalAIEnhancements = VIRTUAL_LAB_MODULES.reduce(
    (sum, m) => sum + m.aiEnhancements.length,
    0,
  );
...
export const getModulesByPhysicsLevel = (
  level: "simplified" | "intermediate" | "advanced" | "research-grade",
): VirtualLabModule[] => {
  return VIRTUAL_LAB_MODULES.filter((m) => m.capabilities.some((c) => c.physicsLevel === level));
};
