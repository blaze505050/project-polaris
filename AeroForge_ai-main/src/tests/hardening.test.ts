...
import { calculateOrbitalPeriodSI, calculateOrbitalPeriodAU, calculateOrbitalPeriod } from '@/services/physicsEngine';
...
export function runHardeningTests() {
  ...
  assert(periodSI > 5500 && periodSI < 5700, 'ISS orbital period should be between 5500s and 5700s');
  ...
  assert(updatedStore.projects.some((p) => p._id === newProjId), 'New project should be added to store');
